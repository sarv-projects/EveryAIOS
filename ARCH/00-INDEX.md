...
> **Workspace count (2026-09-22):** `crates/Cargo.toml` lists **22 members, including `everyaios-engine`** — the crate is **dead (zero dependents)** since `P71.2c` and its deletion is queued (`../TODO.md` P72); no new code may land in it (the post-v1 return vehicle is `P71.7`). Counts of "22 crates" are therefore correct **today** but state the inclusion until the deletion lands, after which the manifest count drops to 21. The `crates/` directory additionally contains non-member entries (e.g. `examples/`, `target/`) that are not workspace members and never counted.
