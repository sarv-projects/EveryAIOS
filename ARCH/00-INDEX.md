...
> acceptance matrix.
>
> **Note (2026-09-22):** the ARCH series is `00`–`15` + the subsystem contracts + `DIAGRAMS.md`. `16` and `17` were moved to `ARCH/archive/` when `ADR/0005` deferred the built-in engine (`P71.5a`); any text that still cites `ARCH/17` as authority is stale and should be repointed to [`AGENT.md`](AGENT.md) / [`EXTERNAL-AGENTS.md`](EXTERNAL-AGENTS.md).
...
16. **Archive** — [`archive/17-NATIVE-AGENT.md`](archive/17-NATIVE-AGENT.md) and [`archive/16-CHAT-LOOP-RUST-PORT.md`](archive/16-CHAT-LOOP-RUST-PORT.md) ⛔ — both **physically archived 2026-09-22 (`P71.5a`)** and neither is a contract. `17` was the v3.75 two-plane contract (its rows B10/B11/C14/C15/F16/I14–I17 remain capability *identity*); `16` scoped a Rust port of the built-in loop. [`ADR/0005`](ADR/0005-external-agents-are-the-v1-engines.md) defers the built-in engine to post-v1, so the loop belongs to the selected agent ([`AGENT.md`](AGENT.md) §1) and there is nothing left to port. Both are kept for the post-v1 governed-baseline return (`P71.7`), not as current architecture. `ARCH/archive/native_loop.rs` (`P71.2e`) sits beside them.
