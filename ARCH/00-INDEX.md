...
> **Docs:** 00–17 define the architecture, module layout, security, routing, memory, caching, office, browser, MCP, algorithms, repository map, UI surfaces, prompt anatomy, visual grounding, connector store, the chat-loop Rust port scope, and **17 = the frozen Native agent plane (two-plane model + tool/agent schema catalog)**. Research provenance and adoption decisions remain in `RESEARCH/desktop_app/`; delivery status remains in `TODO.md`; historical changes remain in `SPEC-CHANGELOG.md`.
> **Decision (user-confirmed):** **Hybrid** — the `@personal-ai/core-*` TypeScript engine (≈100 test files; **vendored in-repo** at `packages/core-*` since 2026-08-29 — the `../APP` sibling and `APP_CLONE_TOKEN` gate are gone) stays as a supervised Bun-compiled sidecar; a **Rust layer owns the paths where research proved Rust wins**: browser/CDP control, script-eval sandbox (rquickjs), security guards, audit/replay ingest, **storage intelligence** (new `everyaios-storage` crate, doc 49). **No scope compromise**: every capability in the research corpus (docs 01–90, **282 repos**) is derived in `09-FEATURE-MATRIX.md` (166 rows). Docs 84–89 add no master-ledger entries: 84 (casual-vs-power UX), 85 (Work Gateway/durable session), 86 (competitor desktop deep-dive), 87 (local-model UX + composer anatomy), **88 (casual-surface / first-five-minutes audit → TODO P61)** and **89 (a code-level audit of our own guard floors → TODO P62, not market research)**. **Doc 90** (native-agent peer-schema provenance → `ARCH/17` / TODO P64) also adds **0 repos**: it grades peer-doc claims (DOC-VERIFIED vs REPO-VERIFIED) rather than surveying new projects.
...
17. **17-NATIVE-AGENT.md** — **frozen 2026-09-15 (spec v3.75; its rows B10/B11/C14/C15/F16/I14–I17 in v3.76):** the EveryAIOS **Native agent plane** vs the **shared cowork plane** — the one invariant (native plane belongs to the agent; shared plane belongs to EveryAIOS), the capability-resolution policy (native-first, augmentation-second), the module-ownership map (one plane + one contract per module), the Chief loop and its edge cases, and the **schema contract for every native tool (`RegisteredTool`), shared façade, agent type, sub-agent spec, prompt segment, and routing rule**.
