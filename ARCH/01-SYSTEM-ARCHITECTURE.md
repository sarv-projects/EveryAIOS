...
**v3.45 Dynamic Chief (agent-native top brain; GovernedSession mechanism corrected v3.46):** the top of the surgical hierarchy is a configurable slot, not a fixed inbuilt engine. `primary_chief` = inbuilt | ACP-installed agent (Claude Code/Claude Agent via official `claude-agent-acp`, Codex via official `codex-acp`). EveryAIOS remains the governed workspace shell — memory passport (C10), Guard-2, audit, tool catalog — whichever agent occupies the Chief slot. Two new contracts on the ACP bridge (J17): **`ChiefAdapter`** (uniform `initialize`/`start_session`/`send_message`/`stream_events`/`cancel`/`request_permission`/`update` over inbuilt + ACP impls) and **`GovernedSession`**. **v3.46 correction (codeg `host_tools_policy` + ACP v2 RFD verified):** omitting `fs`/`terminal` does NOT force MCP Channel B — agents fall back to their own in-process backends (their own sandbox applies; we claim "self-contained", never audit visibility); the governed paths are **Mediated** (advertise fs/terminal:true + service through Guard/path-floor/audit — vscode-acp/codeg pattern) or **Channel B** (our MCP tool catalog); v2 removes the client fs/terminal surface, so the durable path is Channel B + per-agent sandbox config. Per-agent badge = Governed-Mediated | Self-contained | NotGoverned; no boundary/sandbox → no governance claim. The Chief's subagents may be other agents (codex-acp subagent-as-ACP-tool-call); Work survives Chief death via the event-sourced session + `config_hash` (swap Chief mid-Work, resume the same intent→plan→checkpoints→receipt chain).
