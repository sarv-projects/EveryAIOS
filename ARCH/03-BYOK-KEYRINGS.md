...
- **Dynamic scorer (A7):** 13-factor `DEFAULT_WEIGHTS` (health 0.20 / quota 0.15 / costInv 0.15 / latencyInv 0.12 / taskFit 0.08 / …) + 4 mode packs (ship-fast / cost-saver / quality-first / offline-friendly) + `auto/category:tier` model-id DSL — the *static* planner/subagent/writers roles gain a dynamic picker. **Honesty (v3.39):** `tier.rs` holds the strategy vocabulary (`lkgp` parse exists); coordinator `router.ts` is capability-filter + cost-sort. The live loop is `ProviderObservation` history → scorer → `RouteDecision { provider, model, strategy, score, reasons, cost, health, quota, cache_affinity, fallback_chain }` — not yet implemented. Extra OmniRoute modes (round-robin/p2c/fusion/pipeline) stay internal scoring factors, not public architecture.
