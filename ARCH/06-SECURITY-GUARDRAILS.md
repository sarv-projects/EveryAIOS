...
- **Network-destination floor (P62.1, 2026-09-13 — `everyaios-guard::netfloor`).** Before this, three partial SSRF implementations had drifted apart: `toctou::is_blocked_ip` knew only the exact cloud-metadata IP, `urlfloor` had no IP check at all, and the browser engine carried its own private-IPv6 copy — the "N rules, M callers" heterogeneity trap. The classification now lives once and every guard calls it. **Always refused, under every policy** (no opt-in exists): unspecified, link-local (`169.254.0.0/16` — which contains the metadata endpoint — and `fe80::/10`), multicast, broadcast, documentation and reserved/benchmark space. **Policy-gated** (`NetPolicy`): private RFC1918 + CGNAT `100.64.0.0/10` + IPv6 ULA + discovery names (`.local`/`.internal`/`.home.arpa`) are refused by default and need the explicit local-network opt-in; **loopback is allowed by default** on a local-first desktop (local dev servers, Ollama `:11434`, LM Studio `:1234`, the CDP endpoint) but is **refused for destinations chosen by untrusted content** — `NetPolicy::strict()` is what the agent tool path (`tools.rs`), the browser navigation path and any model-derived URL get. Classification is on the **parsed** URL host, so decimal/octal/hex and IPv4-mapped bypass forms are normalized before the check. The module is pure and synchronous by contract (no DNS, no syscalls) — the resolve-and-pin half stays in the TOCTOU binder, which is deliberately not on this path.
- **Protected agent-config surfaces (P62.3, 2026-09-13 — `protected_paths`).** CVE-2025-53773 was a prompt injection that made an agent rewrite its own configuration to enable auto-approve and then execute. The equivalent for us is an agent editing **another harness's** settings, or the ones our ClientCompatibility ring projects into, so `.claude/`, `.codex/`, `.gemini/`, `.cursor/`, `.windsurf/`, `.cline/`, `.roo/`, `.continue/`, `.openclaw/`, `.aider/` and `.mcp.json` join the protected prefixes and are `rm_critical`. **Only settings are protected** — a project's instruction files (`CLAUDE.md`, `AGENTS.md`) stay normal workspace files the user and agent may legitimately edit; over-protecting them would break ordinary work.
- **Ambient credential & secret file protection (CVE-2026-47211, 2026-09-18 — `everyaios-guard::protected_paths`).** Inadvertent or malicious deletion/exfiltration of developer secrets (`.env`, `.env.local`, `.env.production`, `.env.staging`, `.ssh/`, `.aws/`, `id_rsa`, `id_ed25519`, `service_account.json`) is blocked at the Guard-1 kernel floor by registering these secret file prefixes into `PROTECTED_PREFIXES`.
- **MCP stdio launcher sandbox hardening (`everyaios-mcp::npx`).** Stdio server launcher arguments are validated against strict token rules; package arguments starting with `-` or `_` are unconditionally rejected by `trusted_npx_package` to prevent malicious argument/flag injection.
...
| **MCP server child (third-party)** | user-installed MCP stdio servers | **P62.2 (closed v3.74).** `SandboxPosture::{Confined, Ambient}` + `spawn_confined` run the child through `LinuxBwrapBackend` with `--clearenv` + `--unshare-net`, so a confined child inherits **no** ambient provider keys and must obtain credentials through the vault broker; `is_sandboxed()` reports which posture was actually achieved. **The live path is confined:** `mcp_cmds::mcp_attach_commit` → `spawn_with_posture(SandboxPosture::preferred(), …)`, and the `PATH`/`HOME` blocker is solved by `everyaios_guard::sandbox::essential_env()` — a small, explicit allow-list applied **on top of** `--clearenv`, with `spawn_stdio_with_env` refusing any secret-shaped name (`*_API_KEY`/`*_TOKEN`/`*SECRET`/`*PASSWORD`/`EVERYAIOS_*`). A requested confined launch **fails closed** (an error, never a silent downgrade to ambient). Non-Linux is honestly `Ambient` (`preferred()`), the recorded P49.5 platform gap. The ACP harness child already spawns sandboxed. |
...
| **SSRF defaults** | block loopback / RFC1918 / link-local fetches unless `--allow-private-network` explicitly set (env: `OBSCURA_ALLOW_PRIVATE_NETWORK=1`); same policy for our fetch/read path. **P62.1 (2026-09-13): the in-process half is now the one shared classifier** — `everyaios-guard::netfloor` (`NetClass`/`NetPolicy`) is called by `urlfloor`, the egress engine, `toctou` and the browser tier engine, so the launch flags and our own fetch path cannot drift apart | Obscura, doc 89 |
