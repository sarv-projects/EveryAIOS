...
| **MCP server child (third-party)** | user-installed MCP stdio servers | **P62.2 (2026-09-13) — PARTIAL.** `SandboxPosture::{Confined, Ambient}` + `spawn_confined` landed (`everyaios-mcp::attach`) and run the child through `LinuxBwrapBackend` with `--clearenv` + `--unshare-net`, so a confined child inherits **no** ambient provider keys and must obtain credentials through the vault broker; `is_sandboxed()` reports which posture was actually achieved and `preferred()` falls back honestly when bubblewrap is unavailable. **The live attach path still uses the legacy uncontrolled spawn** (`mcp_cmds::mcp_attach_commit` → `AttachedServer::spawn`): `--clearenv` strips `PATH`/`HOME`, which env-based stdio servers need, so flipping the default first requires those servers to be moved onto the brokered credential path (TODO P62.2). The ACP harness child already spawns sandboxed. |
