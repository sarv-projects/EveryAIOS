...
| 1b | **Obscura** (Rust, Apache-2.0, **21K★** — source-verified doc 55; ⚠️ earlier "10K+" stale) | ~70MB binary / **~30MB RSS** | full CDP *server* (14 domains) + custom `LP` (markdown) domain (`LP.getMarkdown`), **embedded MCP server (32 tools, ref-based, 4000-char default text cap)**, `obscura serve/scrape` + `obscura-worker` parallel workers, **SSRF guard (loopback/RFC1918 blocked by default, `--allow-private-network` opt-in), `file://` blocked by default**, bounded queues/max-connections (anti-OOM) | production scraping, built-in stealth-lite (`--stealth`, 3.5K tracker block) | **default Tier-1** (adapt: spawn `obscura serve` via ProcessSupervisor — never build our own engine) |
