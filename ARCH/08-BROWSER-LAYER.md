...
| 1 | **Lightpanda** (Zig, AGPL, beta) | ~123MB peak / 100 pages (**16× less** than headless Chrome), 9–11× faster | CDP :9222 + native MCP | crawling, form-fill, structured extraction | no WebGL/canvas/audio; no native Windows (WSL2); **AGPL → spawn-only, never link**; **default** (`tiers.rs` `#[default]`) |
