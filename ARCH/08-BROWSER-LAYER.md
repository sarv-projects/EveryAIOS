...
- **Engine resolution (tiered — see 8.8):** interactive/authenticated tasks run on **system Chrome → Edge → chrome-for-testing** (fallback, doc 34 §2.1); scrape/RAG tasks run on the **lightweight CDP tier** (**Lightpanda default** per `tiers.rs`, Obscura opt-in — both spawn-only, never linked) when no pixel rendering or login is needed. Launch with `--remote-debugging-port=0 --user-data-dir=<~/.everyaios/browser-profile>` (+ first-run flags); read the real port from `DevToolsActivePort` (never trust a fixed port). All CDP over loopback, token-gated.
...
- **Lifecycle:** on-demand spawn; idle sweep (session retention 60min default, configurable); explicit kill; browser crash → per-tab recovery hooks (doc 03 resume). The Chrome/Lightpanda/Obscura child is a **`ManagedResource`** (install ≠ enable ≠ running ≠ healthy). **Acquisition adapter (v3.39):** HTTP vs CDP vs stealth is the engine's choice, not the model's; cloud browser is not bundled. Learned helpers persist as skills under `agent-workspace/` (E14/P24), not one-shot scripts. **Two surfaces:** this crate is **inbuilt Browse** (CDP). **E9 computer use** is the real OS (path-launch, vision-gated, Manager DAG). Do not drive the user’s Chrome via this CDP child unless they opted into session inheritance (E13). Pixel GUI on native HWND remains E9 (P57 path-launch/background; P59 vision + DAG).
...
> **Expanded in v1.1 (doc 46, BrowserOS 53-tool analysis):** Original 17 core interaction tools + 6 bookmark tools + 5 tab-group management tools + 5 window management tools + enhanced_snapshot (with paint-order filtering, stable refs) = **34 tools**, covering navigation, interaction, content, export, bookmarks, tab organization, and window management; + `file_ops`×3 workspace extension → **37 total** (SPEC E2). Further expansion to 40+ tools possible post-v1 (history search, cookie management, extension control).
> **Post-v1 candidates (doc 55, agent-browser source-read):** `a11y_audit` (embedded axe-core, offline WCAG audits), **annotated screenshots** (numbered labels tied to `@eN` refs), `find` semantic locators (ARIA role + name/label/placeholder), **batch mode** (multi-step command JSON in one call), and a `read` upgrade with **markdown negotiation + llms.txt/llms-full.txt ancestor walk + filter/outline** (no-browser HTTP path).
...
| 1 | **Lightpanda** (Zig, AGPL, beta) | ~123MB peak / 100 pages (**16× less in vendor benchmark** than headless Chrome), 9–11× faster | CDP :9222 + native MCP | crawling, form-fill, structured extraction | no WebGL/canvas/audio; no native Windows (WSL2); **AGPL → spawn-only, never link**; **default** (`tiers.rs` `#[default]`) |
| 1b | **Obscura** (Rust, Apache-2.0, **21K★** — source-verified doc 55; ⚠️ earlier "10K+" stale) | ~70MB binary / **~30MB RSS** | full CDP *server* (14 domains) + custom `LP` (markdown) domain (`LP.getMarkdown`), **embedded MCP server (32 tools, ref-based, 4000-char default text cap)**, `obscura serve/scrape` + `obscura-worker` parallel workers, **SSRF guard (loopback/RFC1918 blocked by default, `--allow-private-network` opt-in), `file://` blocked by default** — and since **P62.1 (doc 89)** the *in-process* half is the one shared classifier `everyaios-guard::netfloor` (`urlfloor`/`egress`/`toctou`/the browser tier engine all call it, so a navigation's destination floor cannot drift from the tool path's), bounded queues/max-connections (anti-OOM) | production scraping, built-in stealth-lite (`--stealth`, 3.5K tracker block) | **Tier-1 opt-in** (escalate path — not the default; `tiers.rs` defaults to Lightpanda until the Obscura binary ships; adapt: spawn `obscura serve` via ProcessSupervisor — never build our own engine) |
...
- **License discipline**: none of these are linked into our MIT/Apache core — all spawned child processes speaking CDP (Playwright for Camoufox), exactly like Chrome today. Lightpanda stays spawn-only (never linked) because AGPL.
...
- **Steel** — ⚠️ **the Rust repo (`steel-dev/steel`) is GONE (404, verified 2026-08-10);** the live project is **`steel-dev/steel-browser`** (7.5K★, Apache-2.0, TypeScript — a Fastify + puppeteer-core *API service*, source-read doc 55). Its *patterns* stay a reference for 8.9 — full storage-context sessions (cookies + localStorage + sessionStorage + IndexedDB with Chrome leveldb decode), casting WebSocket, DuckDB log store — but we implement session persistence ourselves in everyaios-vault, never a dependency.
