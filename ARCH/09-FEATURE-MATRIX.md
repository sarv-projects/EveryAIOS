...
| D4 | **PDF open+edit** | render (pdf.js), form-fill/annotate (pdf-lib), text-swap (lopdf), redact, re-author | sidecar + Rust bridge | 🟡 **NEW** | 04 | **✅ P4.4 landed (engine): `pdf/` — `form::form_fill` (lopdf 0.36, AcroForm `/V` + hierarchical field walk), `replace_text` (exact-match `Tj`), `redact::redact` (`/Redact` annotations), `author::author_pages` (re-author new PDF). 6 pdf tests (100 office), 463 ws tests, clippy 0, fmt clean. pdf.js renderer + free-text/highlight annotation → P4.7.** |
