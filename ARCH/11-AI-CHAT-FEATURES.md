...
| C-2 | **Managed/paid routing** | APP SmartRouter `MANAGED_FREE/FAST/SMART` + server pools | **Remove server pools entirely** (desktop is open-source, no servers). Route = BYOK key-ring → agent-owned provider (the bound agent's own sign-in/provider list); EveryAIOS never inserts a local step into the route. Keep the intent classifier + cache affinity + size-aware reroute (superseded in part by ARCH/16-LOCAL-RUNTIME-INTEROP.md) | v2.0 §0.5: zero server components; ARCH 03 owns key-ring routing |
...
| C-6 | **OAuth subscriptions** | AnythingLLM/Composio managed-auth (01, 13) | BrowserOS-style device-code/PKCE (ChatGPT Pro / Copilot / Qwen) stored in **everyaios-vault**, not cloud proxy; **no third-party managed auth — Connector-platform decision 2026-08-16 (MCP is the platform)** | ARCH 03 §5; local-first hub (13) |
| C-7 | **Composio orchestrator** | `core-connectors` (Lane A/B/C) | **REMOVED — Connector-platform decision 2026-08-16:** cloud SaaS holding OAuth tokens server-side contradicts the local-vault promise; surface = MCP Servers + Native (BYO OAuth/API-key in vault) + Tool Catalog | 13: local hub, no hosted pool |
