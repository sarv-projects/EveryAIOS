...
> **2026-09-25 (retracts the "EveryAIOS Native" row in §4.1a.1 below, which still reads as current):** v1 has no built-in model card. The picker shows the selected agent's own model control when the handshake provides one, otherwise “managed by that agent.” Sign in or Set up appears only for methods that agent returned. A registry row that is not installed is not selectable. There is no EveryAIOS Native peer tab and no global Add key bar for agent login. The live code agrees: `ui/src/components/chat/agent-model-picker.tsx` states “EveryAIOS will not show or inject its Native BYOK/local models here.”
>
