...
- `P71.3a` **landed** — `SubAgentRuntime` (the executor and its `HashMap` of active agents) is **deleted** from `everyaios-blueprint::subagent`; what remains is the pure `DelegationPolicy::admit` (depth → concurrent → total) over a `DelegationGauge`, plus `parent_view()` (the one summary-only projection), `validate_message_endpoints()` (membership supplied by the caller from the graph) and `derive_child_permissions`. `WorkGateway` gained `delegation_gauge()` / `work_depth()` / `presence_is_terminal()`, so depth, active and total are read from the one Work graph: accounting survives a restart and a duplicate is a lookup of the deterministic `<parent>/subagent/<task_id>` id rather than a remembered set. `chat.rs::subagent_rpc` takes the policy as an immutable value and requires `workId` to close a child (`I8`). Remaining gap, stated in `TODO.md`: a spawn that names no parent `workId` is admitted **policy-only** (depth from the spec, `active`/`total` zero, reply marked `accountedFrom: "policy_only"`), and two coordinator dispatch contexts (`chat.ts:1123,1494`) still build `ctx` without `workId`.
- `P71.3b` **landed** — `SwarmSession`/`MemberOutcome` (an accumulating driver) deleted from `everyaios-blueprint::swarm`; `SwarmSpec`/`SwarmMode` stay as the strategy plus a **pure** `reduce(spec, results)` over child-Work results (I9).
- `P71.3c` **landed** — `everyaios-core::automation_runtime` converted from executor to **Work factory** (`AUTOMATION.md` §5): `compile_work(&automation, revision_id, occurrence_id) -> WorkSpec { provenance, objective, steps, capability_requests, agent_required }`; validation at compile time; no effects, no retries, no execution state. (`P71.3c` row carries its full marker.)
- `P71.3d` **landed** — `everyaios-core::scheduler_service` rewritten as the **trigger plane** (`AUTOMATION.md` §9): `RunState` (leases/fences/checkpoints/retry), `current_run`/`RunSnapshot`, the runs ledger, drift pins and `has_api_key`/`dispatch_preflight` all deleted; `mark_fired` (occurrence + dedupe + frequency + schedule advance) added; pause is a trigger flag; `scheduler/lease_*` RPCs gone, `scheduler/mark_fired` added, `scheduler_runs` Tauri command deleted, `scheduler_continuity` → `scheduler_notepad_get`. Coordinator `scheduler.ts` is a trigger-plane client (ticks due, runs through the chat engine, records the firing) with a rewritten test suite; UI jobs carry `paused`/`lastFiredAt`/`recentFires` and the mock runs ledger is deleted (History points at the Event Log). Side-findings fixed: `delegation_gauge` per-parent counting, kernel-routed façade test, ACP lifecycle test vs P69.C1's fail-closed gate. (`P71.3d` row carries its full marker.) **This row is test-verified at the touched scope** (`cargo test -p everyaios-core` 744/744; scheduler unit 30/30; coordinator scheduler 8/8; tsc clean; IPC parity 350 registered, 0 broken) while the **wave as a whole remains unverified end-to-end**.
- `P71.3e` **landed** — `everyaios-core::multirun` is agent/run-centric (no model list): `MultiRun { id, work_id, agent_ids, run_ids, worktree_ids, mode }` over Runs of **one Work**, member Run ids derived in Rust (`<work_id>/run-<i>`) so no sidecar mints a kernel id, Work required + budget 1..=5 Runs (1:1 worktree when supplied); `RunOutcome { run_id, agent_id, output, score }`; `FuseMode::Fuse` attributes `## <agent_id> (run <run_id>, score …)`. `execution/multirun` takes `workId`/`agentIds`/`worktreeIds` and refuses outcomes naming an agent or Run outside the fan-out; coordinator `dispatchMultiRun` mirrors it and the subagent tool arg `models` → `agents` (ADR-0005 — an external agent owns its own model). (`P71.3e` row carries its full marker.) **Targeted tests green** (`--lib multirun` 7/7, `bun test src/tools.test.ts` 18/18, `tsc --noEmit`, workspace + `src-tauri` checks).
- `P71.3f` **landed** — one canonical readiness state (`everyaios_types::AgentReadiness`, `ARCH/AGENT.md` §3.1) replacing scattered booleans. Declared once with helpers + snake_case wire; read by the **delegation gate** (`DelegationPolicy::admit` judges readiness first, emits `SubAgentError::NotReady` naming the state, wired through `DelegationBridge` → `subagent_rpc` → the relay arm), the **picker** (`AgentDirectoryEntry.readiness` replaces `installed: bool`; `agent_directory_list`/`acp_install_status` emit `readiness`; the UI derives usability from it), the **scheduler** (trigger-plane doctor `agents` check; firing turn's `start_stream` gate emits `ChatRelayError::AgentNotReady`) and the sidecar (`agent/readiness` RPC). One source: `AgentReadinessSource` seam + `ChatRelay::mount_readiness`, mounted at boot by `ShellAgentReadiness` (install/discovery facts + live ACP handshake auth/session). (`P71.3f` row carries its full marker and its remainder: `Failed`/`Unavailable` await a launch-attempt record; the per-session firing agent is P71.8/P71.5b.)
- Status: **compile-level only for `P71.1`/`P71.3a`/`P71.3b`/`P71.3c`/`P71.3f`** — no suites were run for those rows (per the standing "implement first" instruction); `P71.3d` + `P71.3e` additionally carry the targeted Rust/TS test runs named above (one load-flaky RSS test passes in isolation). Gates: `check-doc-sync` / `check-arch-invariants` / `gen-codebase-map --check` exit 0 after the doc sync below.
- `P71.3g` **landed** — the Work-state contract in code: `everyaios_types::WorkState` carries the full §4 lifecycle (four `Waiting*` sub-states, `Verifying`, and `Recoverable` as a first-class resumable outcome); `WaitReason`/`WaitCondition` (`AUTOMATION.md` §8) and `CheckpointId` (`ckpt:<work>/<step>`) exist and are stamped. The Work Gateway's transition door is typed (`record_execution_transition(WorkState)`, `record_wait`, `finish_child_work(WorkState)` — non-terminal child outcomes refused where the old door defaulted them to `completed`); `RunInterrupted` exists so a recoverable run is never a failed run; the `execution/transition` RPC refuses unknown state spellings; presence carries `work_state` + `wait` and projects `Recoverable` as blocked, never failed (I15). UI labels the new states and renders interruptions as resumable. (`P71.3g` row carries its full marker; the coordinator's own turns do not emit waits yet — the door is open for P71.8.)
- `P71.8a`–`d` **landed** — session kinds: `SessionKind { Interactive · Automation · Delegated }` in `everyaios-types` (Default `Interactive` — existing rows need no migration), carried as a record property on `WorkAddress.session_kind` and settable only **explicitly** (`create_work_in_session` / `work/create`'s `sessionKind`). The kernel refuses an automation Work without its owning Session (ADR-0006 §4 as an error — no Work outside the scope chain, **I4**), child Work inherits the parent's Session/kind (§5, **I8**), the coordinator's trigger client creates the firing Work as `sessionKind: "automation"` with no Chat (§2 — no hidden Chat per run), the UI gained `sessionOwnerSurface` + an honest History note, and the contract test encodes refuse → create → inherit. (`P71.8` rows carry their markers; the run-list rows from the Event Log projection and the open-a-run → attach-a-Chat action are the verification-pass remainder.)

- **First full-suite verification pass over the P69/P71 waves (2026-09-22; `SPEC-CHANGELOG.md` v3.96). 0 flips.** Every suite in the repository was run for the first time since the external-agent consolidation began, and it is **green**: `cargo test --workspace` **2691 passed / 0 failed / 25 ignored**; `bun test` ui **383/0**; `bun test` coordinator **238/0**; `src-tauri` `cargo check` clean; `cargo clippy` on the touched crates 0 warnings; `tsc --noEmit` clean (ui + coordinator); `check-doc-sync` ✅ (166; 1640 = 1304 + 336; v3.96); `check-arch-invariants` ✅; `ipc-parity` 350 registered / **0 broken**; `gen-codebase-map --check` ✅; `scripts/e2e/security-gate.mjs` **PASS** (S1–S6). **Five real defects were caught in code that already carried an `[IMPLEMENTED]` marker, and all five are repaired:** (1) `ui/src/lib/autonomy.test.ts`'s `taskScopeHash(a) !== taskScopeHash(b)` had been collapsed to `x !== x` by the built-in-id rename — both operands had become `'claude-code'` — so a *correct* test was failing; restored to two distinct agents. (2) `packages/coordinator/src/h32.ts`'s `isAcpAgent` still returned `true` for the retired `'everyaios-native'` spelling, leaving the deleted engine a live entry point; it is now refused by name. (3) `AgentDirectory::upsert_bundle` returned `HashMap::insert`'s **previous** value, so a first insert and a rejected draft both read as `None` — "created" and "refused" were indistinguishable; it now returns `Some` when the row is admitted. (4) `crates/everyaios-core/src/chat.rs`'s relay used `unwrap_or("inbuilt")` for an absent harness, quietly satisfying `bind_runtime`'s fail-closed *harness required* check and re-admitting the retired engine through the default path; the relay now fails closed, and its fixtures name a real agent with mounted readiness. (5) The MCP external-client tests (`p10_external_client.rs`, `external_client.rs`) demanded ≥40 tools including `snapshot`/`search_web`/`office_edit`/`memory_retrieve` — the **internal** catalogue that `P64.9`/`P71.2f` deliberately stopped advertising. An external client is served the **shared plane** (`everyaios_mcp::SHARED_FACADES`, 19 task-shaped façades per `ARCH/EXTERNAL-AGENTS.md` §3), with internal ids callable by name but never listed; the assertions were inverted to match and now **assert the primitives are absent**. The same stale claim was corrected in the standalone server's module doc and in the `initialize` instructions string. **Plus one test-robustness repair:** `everyaios-vault`'s `cooldown_backoff_doubles_and_caps` bounded itself with a fixed ±10 ms wall-clock slack that a fully-parallel run outruns; it now derives an exact range from the window bracketing each call (no magic slack, no vacuous bound). **Files:** `ui/src/lib/autonomy.test.ts`, `packages/coordinator/src/{h32.ts,primary-agent.test.ts}`, `crates/everyaios-agents/src/directory.rs`, `crates/everyaios-core/src/chat.rs`, `crates/everyaios-mcp/tests/{p10_external_client.rs,external_client.rs}`, `crates/everyaios-mcp/src/bin/mcp-standalone-server.rs`, `crates/everyaios-mcp/src/server.rs`, `crates/everyaios-vault/src/keyring.rs`, `CODEBASE-MAP.md` (regenerated). **Not claimed:** row-level acceptance. The 39 `[IMPLEMENTED — unverified]` markers keep their labels and stay counted open — a green suite is evidence about the tree, not proof each row's own criteria were demonstrated. Still unrun: the `P70` release-engineering block, `P66.5`'s global visual/DOM gate, and the Windows acceptance work (`P70.D5`/`D6`).
- **Cockpit de-inbuilt'ed (`P71.9a` IMPLEMENTED — unverified, `P71.9c` PARTIAL, 2026-09-22; `SPEC-CHANGELOG.md` v3.85).** The UI no longer contains a built-in agent, so the picker can no longer look satisfied while the send path refuses. `ui/src/lib/agents.ts` drops the `everyaios-native` row plus `NATIVE_AGENT_ID`/`isNativeRuntime`/`modelsForUsableRuntimes`; `isRuntimeUsable` loses its "always live" shortcut; `getModelsForAgentLive` returns `[]` for every runtime (`P71.2d`); `DEFAULT_ROUTING` names no agent (empty = the session's bound agent). The store starts unbound, `setSelectedAgent` is id-only, `cycleModelVariant` is a no-op, `isRetiredBinding`/`currentBinding` live in `acp.ts` as the one predicate the turn path, the composer and the picker share. `agents-models-section` loses its built-in card and the 262-line `NativeModelCatalog`; status-bar paints the agent's own ACP model or "managed by <agent>"; command-palette loses the desktop model rows and the cycle; local-models-panel / runtime-session-section / onboarding-modal / analytics fixtures de-inbuilt'ed; `packages/coordinator/src/h32.ts` restated (nothing is ever forwarded). Validated: `tsc --noEmit` clean for `ui` + `coordinator`; doc-sync ✅ (1640 = 1304 + 336); arch-invariants ✅; codebase-map ✅ (1470 files, 335 commands); ipc-parity 0 broken. Tests rewritten, **not run**: `agent-ownership`, `occupancy`, `h32`, and the DOM suites `status-bar` / `agents-models-section` / `agent-model-picker`. Declared-not-stale remainders were resolved by v3.86 (`P71.9c` complete): the dead `!external` JSX, the local rows and the routing-table gap are all landed.

- **Doc sweep (2026-09-22, no code change).** `CODEBASE-MAP.md`'s hand-authored narrative now matches the post-`P71.2c` tree: §1.3 (keys never cross the wire; the provider broker is archived), §1.4 module table, §1.5 Flows 1–6 (the ACP turn path with readiness + budget re-homed at `acp_prompt`; automations fire in the host; CUA is Rust-side), §2.1 (the live shell↔sidecar method set), §2.3 (the loop is the bound agent's), §2.4/§2.5 (prompt assembly and the TS tool catalogue archived), §2.7 (the broker is not a model gateway), §3.1 (the retired methods, named), plus the command count (349 → 345). `ARCH/archive/coordinator-loop/README.md` gained **"the loop's environment"**: the 40 modules whose only importer was the archived loop, with the two re-home exceptions (`run-identity` → `P71.9c`, `work-events` → `P71.9e`) and the MCP trio; `TODO.md`'s `P71.2c` row carries the same inventory as its narrowing pass. The `everyaios-engine` crate's module docs no longer point at live `packages/core-engine` paths (its TS reference is archived; the crate is kept as pure policy per `P69.D27`). No suite run.

- **Appearance has one owner and every offered theme is painted (UI audit + repair, 2026-09-22; `SPEC-CHANGELOG.md` v3.95).** Six defects found in already-landed appearance code: `AppearanceSection` was a second writer of `font-scale-*`/`high-contrast` from its own `usePref` (so the choice only held while the panel was mounted); the theme/accent keys were stored **double-prefixed** (`everyaios.settings.everyaios.theme`) while `index.html` read the unprefixed ones, so first paint always flashed the default and the shipped `theme-persist.test.ts` was **failing 2 of 5**; `rose`+`teal` were offered with **no `[data-accent]` block** in `globals.css`; `onboarding-modal` kept a third accent list two presets behind; `data-density` was written and read by no stylesheet; the picker offered only light/dark while the default mode is `system`. Repaired to one owner (`ThemeProvider`) + one encoding (plain text at literal keys, legacy forms still read); `rose`/`teal` tuned light+dark and added to `design-tokens.test.ts` so an unpainted preset now fails a test; density implemented off Tailwind v4's single `--spacing` token in three steps; `system` mode selectable with the resolved theme named, plus `resetAppearance`; icon-only theme/sidebar/accent controls given accessible names and pressed state. **Files:** `ui/src/lib/ui-prefs.ts`, `ui/src/components/{theme-provider.tsx,onboarding-modal.tsx}`, `ui/src/components/panels/{settings-sections.tsx,settings-panel.tsx}`, `ui/src/components/shell/title-bar.tsx`, `ui/src/globals.css`, `ui/src/lib/design-tokens.test.ts`, `ui/index.html`, `ui/src/main.tsx`. **Verified:** `tsc --noEmit` clean; `bun test src/lib/design-tokens.test.ts src/lib/theme-persist.test.ts` → **15 pass / 0 fail**. **Not run:** any other suite; P66.5's global visual gate stays open.

- **v1 is a Windows product, and the matrix says so (P70 scope decision, 2026-09-22; `SPEC-CHANGELOG.md` v3.94).** v1 ships for **Windows** (`.msi` WiX + `.exe` NSIS, x64 + arm64); the Linux half ships as **WSL2** (a supported host for Linux-native agents, not a cockpit artifact); **macOS and native Linux desktop are out of v1 scope**. Reason is evidence: desktop control is host-dependent and the only verifiable host is Linux, which v1 does not ship — so Windows is where the acceptance work belongs, and WSL keeps Linux-native agents reachable from that machine. Listed `/home/sarvesh/business_Dev/desktop_app/SUPPORT-MATRIX.md` (**new**) is the published form (per-platform matrix, Windows specifics incl. the weaker sandbox posture stated rather than equated with `bwrap`, an explicit *what is not claimed* section for unverified Windows capture/UIA/ConPTY, data/upgrade policy, and where the decision is enforced); `ARCH/DESKTOP.md` §6 rewritten as its architectural half (§6.1 decision + host-evidence rule, §6.2 gaps incl. ConPTY); `TODO.md`'s P70 preamble gained the scope rule with its per-row consequences (`A1`/`A3`/`A4`/`A5`/`A6`/`B1`/`B5`/`B6`/`B7`/`C*`/`F1`/`F2` Windows-only; `B2`/`B3` **OUT OF V1**; `D5`/`D6` answered; `E5`/`E9` Windows + WSL). `tauri.conf.json` deliberately still lists all six targets — the local harness builds and boots here; `P70.A1` narrows what CI *publishes*. Gates: doc-sync ✅ (v3.94), arch-invariants ✅, ipc-parity 0 broken, codebase-map ✅.

- **The engine is provably optional (`P71.6b` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.93).** New `crates/everyaios-acp/tests/acceptance_engine_optional.rs` (4 tests, no mocks, no credentials, no network): a real spawned `mock_acp_agent` completes an initialize → session/new → session/prompt turn with nothing of ours in the path and its usage report survives onto `PromptOutcome` and into the real `UsageLedger` as `UsageSource::AgentReport`; a `no-usage` prompt is asserted **unreported with zero totals** (I15 against a live process); exhaustive matches over `AgentProtocol`/`HarnessProtocol` make an `Inbuilt`/`ModelBackend` reintroduction a compile error; and the deletion is asserted **on disk** (native loop archived + absent from the crate tree, `packages/core-engine/src` and `packages/coordinator/src/chat.ts` gone). The `mock_acp_agent` fixture now reports usage; `everyaios-acp` gained `everyaios-memory` as a dev-dependency. **Not run** (standing instruction) — compile-checked only; verification command `cargo test -p everyaios-acp --test acceptance_engine_optional`. Reminder: `gen-codebase-map.mjs` enumerates tracked files, so regenerate the map in the commit that adds this file. Gates: `cargo check -p everyaios-acp --all-targets` clean, doc-sync ✅ (v3.93), arch-invariants ✅, ipc-parity 0 broken.

- **Usage is an observation with its reporter named (`P71.4` + `P71.9h` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.92).** P71.2c deleted the broker that *measured* a turn, leaving the ledger with **no live producer** (its only callers were tests). The ledger can now not be written without naming a reporter: `everyaios_memory::UsageSource` (`agent_report` · `acp_event` · `provider_report` · `capability_call`) + `UsageObservations`; `UsageRecord` gained `cached_write_tokens` (a cache write is billed apart from a read) and `reported_cost_usd` (strictly apart from `est_cost_usd` — producer figure vs our estimate); `record_observed(source, …)` is the only door and `record_unreported(owner)` records the opposite fact (a turn with no report is absent, never zero); the primary/worker split moved to the **agent** dimension and the `contains("chief")` key-name heuristic is deleted (`null` when unattributed). **Live producer:** `SessionPromptResult` parsed only `stop_reason` — it now carries `PromptUsage` through `PromptOutcome`, and `acp_prompt` records the turn as `AgentReport` or unreported. **UI:** `spend.ts` (`usageSourceLabel`/`unreportedOwners`/`costReadout` with `reported|estimated|unknown`), Settings → Usage labels each figure and lists turns with no report, Analytics drops the invented share fallback and labels its badges. Gates: `cargo check --all-targets` clean (memory/core/acp/src-tauri, 0 warnings), ui `tsc --noEmit` clean, doc-sync ✅ (v3.92), arch-invariants ✅, ipc-parity 0 broken, codebase-map ✅. No suite run.

- **No affordance offers a built-in engine (`P71.9g` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.91).** The wizard was already v1-correct (`EngineBinding` = `acp | model-only`, unbound saves as a draft), so the row landed the rest of the tree: `ui/src/lib/settings.ts`'s `AgentProtocol` (`'inbuilt' | 'acp' | 'mcp'` — a hand-maintained duplicate missing the shell's real `model_only`) is now the canonical `'acp' | 'model_only'` mirroring `everyaios_types::AgentProtocol`, and `nativeSurfaceNotReplaced` tests `'acp'` only; `discovery.ts`'s demo inventory dropped the `id:'inbuilt'` EveryAIOS card (`capabilitiesVerified: true`, `status: 'healthy'`) that presented a runnable engine no install can produce (`counts.agents` now 0); `doctor.ts`'s "42-tool inbuilt catalog always available" became the bundled catalog as *discovery data*; `runtime-bind.ts` refuses a bind that names no harness instead of defaulting to `inbuilt` (module stays archive-pending per `P71.2c`); brand vocabulary removed from `store.ts`/`chief-handoff`/`bridge`/`chat-composer`/`agent-model-picker`/`connectors-panel`/`cua-route`/`settings-panel` and the DOM harness fixture (`chief_default_get` now answers unbound). Surviving `inbuilt` spellings are retired-value guards only. Gates: ui + coordinator `tsc --noEmit` clean, `check-doc-sync` ✅ (v3.91), `check-arch-invariants` ✅, `ipc-parity` 0 broken, `codebase-map --check` ✅. No suite run.

- **Headless runs reached through their owner (`P71.9f` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.90).** `ADR-0006` §6/§7 surfaced. **Open a run:** `openAutomationRun(run)` in `ui/src/lib/store.ts` (+ `originRunId`/`originRunSessionId` on `Session`) creates a Chat titled from the run's own Work objective and carrying the run's identities, switches to chat, and says it *continues* that run — the run list stays the surface that owns headless work and no Chat is fabricated per firing; `automations-panel.tsx`'s `LiveRuns` row gained the **Open** action. **The owner is named, not the container:** `work.ts`'s new `sessionKindLabel(kind)` is the one user-facing projection (`chat` · `automation` · `delegated`); `tasks-rail.tsx` no longer prints the raw wire enum and `progress-view.tsx`'s Live Work block lists each Work with its owner (up to six). **Vocabulary sweep:** the word "Session" is gone from every rendered string (`title-bar`, `right-rail`, `shell-view`, `settings-panel`, `runtime-session-section`, `usage-metrics-section`, `settings-sections-studio`, `settings-sections-security`, `guard-panel`, `chat-composer`); remaining spellings are identifiers/type names/wire transforms only. Declared: the §7 chat is a fresh transcript (the run's event history stays the Event Log's surface). Gates: ui `tsc --noEmit` clean, `check-doc-sync` ✅ (v3.90), `check-arch-invariants` ✅, `ipc-parity` 0 broken, `codebase-map --check` ✅ (1466 files). No suite run.

- **Automations show honest run status (`P71.9e` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.89).** The `AUTOMATION.md` §11 surface is real, reading the ledger that owns run history rather than asking the trigger plane to keep a second one. Rust (`scheduler_cmds.rs`): `scheduler_runs(jobId)` reads the ExecutionLedger through a new read-only `ExecutionKernel::all()` accessor and returns the `Trigger::Scheduler` rows (filtered by the `"automationId":"<id>"` needle in `context_snapshot` — the exact JSON `scheduler_fire.rs` writes) newest-first, capped at 50, each as `{id, sessionId, objective, phase, waitingApproval, createdAtMs}` — the ledger's own `ExecutionPhase`, no fabricated success; `scheduler_duplicate(id)` clones under a new id with `enabled: false` (a duplicate never silently arms a second trigger); `scheduler_export(id)` returns the `*.automation.json` body with `boundAgent` resolved exactly as the firing path resolves it (retired spellings → `null`) and steps carrying capability *requests*, never credentials. UI: `schedulerRuns`/`schedulerDuplicate`/`schedulerExport` wrappers; the History tab's pointer-card became a real runs list with per-phase styling (`waiting for approval` / `running` / `completed` / `failed` / `cancelled`); rows gained **Duplicate** + **Export** (blob-download `<id>.automation.json`), completing the §11 action set; the card meta shows the bound agent via `sessionAgentLabel()` (session pin → user default, retired-filtered) or "none bound". Gates: src-tauri `cargo check` clean 0 warnings, ui `tsc --noEmit` clean, `check-arch-invariants` ✅, `check-doc-sync` ✅ (v3.89), `ipc-parity` 0 broken. No suite run.

- **Delegation visible + configurable (`P71.9d` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.88).** Rust: `SubagentPolicy` on Config (B3 defaults) + `chief_subagent_set_policy` + `work_children` (gateway `children_of`, read-only), registered. UI: `SubagentProfile`/`chiefSubagentSetPolicy` (`acp.ts`), `workChildren` (`work.ts`); Settings → Subagents rows gained a **Delegation profile** disclosure (role · model policy · may-spawn · caps · budget · workspace; the gateway's live gauge stays the admission authority); `tasks-rail.tsx` renders **child work** per row best-effort. Declared: a dedicated Work-detail pane and per-profile `allowed children` consumption are verification-pass remainders. Gates: src-tauri check clean, ui tsc clean, ipc-parity 0 broken (347), doc-sync/arch-invariants ✅. No suite run.

- **`Chief` retired to Primary Agent (`P71.5b` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.87).** `chief.ts` + suite **deleted**; `primary-agent.ts` carries the fail-closed resolution (explicit → user default → **none**, no `inbuilt` fallback; retired spellings refuse by name in `validateSessionPin`), the unchanged delegation limits, and the Work-survives-agent-death registry; `index.ts`'s `chief/*` arms drop the inbuilt branch and report `source: "none"`. Rust: `WorkerOutcome`→`DelegationOutcome`, `AgentRole`→`DelegationRole`, `ChiefSpend`→`PrimarySpend` (`split_primary_spend`, `PRIMARY_SPEND_WARN`), `RuntimePlane::Chief`→`Primary` — callers and docs updated; the `usage_snapshot` wire key stays `chiefSpend` until the `P71.4` re-home (commented both sides). UI: `PrimarySpendView`/`primarySpendWarning`, "Default agent" picker slot, "Primary agent is not delegating" badge, Subagents/routing/right-rail copy. Gates: cargo check workspace + src-tauri clean, `tsc --noEmit` clean (ui + coordinator). No suite run.

- **The composer is the bound agent's surface (`P71.9c` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.86).** The last EveryAIOS-owned chat control is gone. `agent-model-picker.tsx`: the dead `!external` branches are **deleted, not guarded** — models.dev catalog rows/notes, curated seed, local-model rows, auto-route toggle + route feed, the catalog/cloud pin handlers, their three effects, the unused store selectors (`selectedModelId`/`setSelectedModel`/`autoRoute`) and imports (1707 → ~1150 lines); the model column is the agent's own ACP option or an explicit "none exposed", and the trigger chip paints the agent-owned value or `—`. `chat-composer.tsx`: `runSlash` (the local `/help /mode /model /undo /compact /clear /export` dispatcher) and the static-table hint fallback removed — `/word` is text for the agent, suggestions come only from `available_commands_update`. **`ui/src/lib/slash-commands.ts` + suite deleted**; Settings lost the Commands section and nav entry. `agents-models-section`: the routing table's empty value is the named `ROUTING_BOUND_AGENT` option ("follow the bound agent", new export in `agents.ts`), and the tab header no longer names a built-in engine. **Incident recorded:** `settings-sections-studio.tsx` was briefly truncated by a bad tool write mid-edit and restored from git HEAD with the v3.85 wording re-applied (v3.86 documents it). Gates: ui `tsc --noEmit` ✅ · doc-sync ✅ (1640 = 1304 + 336) · arch-invariants ✅ · codebase-map ✅ (1468 files) · ipc-parity 0 broken. No suite run.

- **First run leads with the agent (`P71.6a` + `P71.9b` IMPLEMENTED — unverified, 2026-09-22; `SPEC-CHANGELOG.md` v3.85 "Also in this wave").** The old first-run gate completed the *zero-install* path (paste a key or start a local runtime) — a promise `P71.2c`/`P71.2d` made impossible to keep. `ui/src/components/shell/setup-gate.tsx` rewritten: cloud-key and local-model modes removed (they stay in Settings as *observation*), discovered agents listed with their canonical readiness (`readinessLabel`, one owner in `acp.ts`) and per-row `Use` · `Sign in` (the agent's own method; nothing copied) · `Install` (the picker's Guard-2 handshake); binding = `chief_default_set` + republished user default + `setSelectedAgent`, so the shell stays the authority. `chat-panel.tsx`'s `NoProviderCard` → `NoAgentCard` (blocker is now 'no agent bound', checked via `currentBinding`). `onboarding-modal.tsx` step 2 stopped claiming a native engine and stopped badging discovery as ready; it gained **Use this agent** and the missing Guard-2 consent on install, plus a `!boundId` banner. Validated: `tsc --noEmit` clean; doc-sync ✅ (1640 = 1304 + 336); arch-invariants ✅; codebase-map ✅; ipc-parity 0 broken. No suite run. Remaining for the first-run story: the clean-profile boot (`P70.D2`) and a DOM test for the gate (`P70.E2`).
- `P71.2a`/`P71.2b` **landed** (2026-09-21) — the built-in agent identity is retired and the "point this CLI at my models" path is gone. Code: `HarnessProtocol::Inbuilt` + `ModelBackend` deleted (`Acp` is the whole v1 vocabulary), `is_default` removed from `HarnessManifest` and `AgentDefinition`, `AgentSource::Inbuilt` removed (the picker's `default_entry` is now the first *ready* row — no assumed default), `EngineBinding::Inbuilt` removed with `AgentBundle.engine` now `Option<EngineBinding>` (`None` = draft: `definition()` returns `None`, `upsert_bundle` refuses it, so a bundle with no brain never joins the runnable directory), `launch_plan` no longer takes a backend URL and **never** injects a model endpoint, `acp_agents` reports `SelfContained` for every row, no id gets a mediated ACP session, `openai_cmds` model rows name their real owner instead of the host, the picker force-install + `ACP_TO_CATALOG["everyaios"]` are gone, the mention seed drops `@everyaios`, templates pre-fill no brain and the wizard refuses to save an unbound bundle. `AgentProtocol` is now `Acp` · `ModelOnly` (the latter is a v1 model-only *bundle*, not an engine). Guard principals named `everyaios` are untouched (ADR-0005 §7). **Open, recorded:** a `model-only` bundle has no v1 executor once `P71.2c`/`P71.2d` remove the sidecar inference path, while `DESKTOP-APP-SPEC` B9 and `P71.9g` keep it as a choice — the verification pass must either re-home it or mark it post-v1; and the UI picker still defaults to `'inbuilt'` locally (P71.9a).
- `P71.2c` **partially landed** (2026-09-21) — **the composer no longer has a built-in fallback**. `ui/src/lib/bridge.ts` resolves the session's **bound agent** (session pin → user default → selected agent) and, when nothing is bound, opens the agent-discovery screen with a stated reason instead of substituting an EveryAIOS-owned engine; the retired spellings (`inbuilt` · `everyaios` · `everyaios-native`) resolve to **nothing** rather than to a substitute (ADR-0005 §1/§2). The BYOK provider gate is gone from the send path — an external agent owns its own credential, so a missing EveryAIOS key no longer blocks a turn. Pause/Stop now cancel the bound agent's live ACP session (`acp_cancel`) because the native provider stream and its `chat_cancel` handle are part of the doomed path; the composer's `/` intercept follows the same rule (a bound agent's own vocabulary wins, an unbound session keeps the local table). A chat scoped to an open document (P33) now travels to the agent as labelled prompt text, since the native prompt compiler that wrapped it (J6) is going with the loop — dropping the document silently would be worse. `chatStream`/`chatCancel` TS wrappers deleted. **Still to remove in this row:** the coordinator loop (`runChatStream`/`runInbuiltTurn`, `ConversationEngine`, `FrameProviderBridge`), `core-engine`'s engine/stages/trajectory/automation-engine, and the Rust `ChatRelay` provider broker + `provider/stream` arm + `start_stream`/`chat_stream`. **Recorded consequence:** the broker's removal also takes the plan *executor* (its LLM turns run on that broker) — plan execution therefore returns with the governed binding post-v1, while the UI's read-only plan draft is unaffected. `tsc --noEmit` clean for `ui`.
- `P71.2d`/`P71.2e` **landed** (2026-09-21) — native inference and the native loop are gone. `P71.2d`: `packages/core-providers/{openai-client,anthropic-client}.ts` deleted with their tests and every export they carried (`streamCompletion` · `streamAnthropicCompletion` · the key probes · `fetchAvailableModels` · `ANTHROPIC_KNOWN_MODELS` · the `OpenAiProviderConfig`/`ValidationResult` shapes); **the observability half stays** — provider catalogue, the generated model catalogue + capability helpers, live pricing and the `ProviderVault` façade (custody remains Rust-side). No consumer existed outside the deleted tests; the coordinator smoke test now asserts the removal (`bun test src/core-providers.smoke.test.ts` 6/6, package rebuilt, `tsc` clean). `P71.2e`: `crates/everyaios-core/src/native_loop.rs` moved to `ARCH/archive/native_loop.rs` (banner names ADR-0005 §4, the post-v1 return note and the `DirectGuard` → `everyaios-guard` rule), the module registration removed, and the migration ledger gained `TierStatus::Archived` for tiers 1a/1b so it no longer advertises a collapse whose consumer is gone.
**(Now, 2026-09-22 — P71.2c/P71.2f landed: the built-in engine is deleted, and `ARCH/17`/`ARCH/16` are physically archived)**:
- `P71.5a` **landed** — `ARCH/17-NATIVE-AGENT.md` and `ARCH/16-CHAT-LOOP-RUST-PORT.md` moved (with history) to `ARCH/archive/`, each carrying an ARCHIVED banner naming `ADR-0005`, the post-v1 return (`P71.7`) and the move date, with relative links rewritten. Every live citation repointed: `ARCH/00-INDEX.md` (reading order + the document-accounting table now carry an **Archive** row; the series is stated as `00`–`15` + contracts + `DIAGRAMS.md`), `ARCH/02`, `ARCH/11`, `ARCH/12`, `ARCH/13`, `ARCH/15`, `ARCH/DIAGRAMS.md`, `README.md`, `DESKTOP-APP-SPEC.md` §4.6, `UI-DESIGN-PROMPT.md`, `UX-TESTING-PLAN.md`, `docs/codebase/{components,decisions}.md`, `TODO.md`'s P64 heading + row-table label. `ARCH/16` joins the move because it scoped a Rust port of a loop that no longer exists.
- `P71.2c` **landed** (the wave's one large surgery) — **the sidecar reasoning path and the Rust provider broker are deleted, not bypassed.** TS: `packages/core-engine` left the workspace for `ARCH/archive/core-engine/`; fourteen coordinator modules (the loop, the plan executor, the native catalogue, the prompt assembler, mention/context-provider/intent/edit-strategy/chunking/citation helpers) moved to `ARCH/archive/coordinator-loop/` with their suites and a README each; `index.ts` lost every `chat/*` and `plan/*` arm and the provider bridge. Rust: `start_stream`, `ChatStreamParams`, `retry_tool`, `cancel`, `cancel_session`, `start_plan`, `respond_plan`, the `provider/stream` arm, the 186-line `stream_provider` broker and the relay's whole provider dial plan are gone; the shell lost `chat_stream`/`chat_cancel`/`chat_tool_retry`/`plan_execute`/`plan_respond` + the boot-time endpoint/profile sync. **The readiness gate (P71.3f) and the J11 budget pre-flight were re-homed to `acp_prompt`** — the live turn boundary — via the new `ChatRelay::preflight_session_budget`; `agent/stop` cancels live ACP sessions. `LAYER-1` now fails the build if `packages/core-engine/src` returns or `ConversationEngine`/`runChatStream` is re-declared outside `ARCH/archive/`. **Compile-level + gate-level only** (no suite was run, per the standing instruction): `cargo check --workspace --all-targets` + `src-tauri` clean with 0 warnings, `tsc --noEmit` clean, `check-arch-invariants` OK, `check-doc-sync` ✅, `codebase-map --check` ✅, `ipc-parity` 0 broken (345 registered, was 350). Four Rust suites were **re-pointed** at the contracts that survived and four deleted with their subject; `crates/everyaios-core/tests/p50_e2e.rs` was deleted with its CI step (its live replacement is `P70.E5`).
- `P71.2f` **landed** — the native catalogue is not "narrowed" but gone: after `P71.2c` it had no consumer, and the canonical façade table was always Rust-side (`everyaios-mcp::SHARED_FACADES`) with the kernel registry (`everyaios-core::tools`) as the only executor. A TS duplicate of the façade list was the thing the row existed to prevent.
- **Declared, not deleted** (so nothing reads as stale): the Rust `chat/*` receive plane (`ChatWireEvent` → `on_event`) and the UI's `onChatEvent` handlers have **no producer** now — `P71.9c` feeds them from the ACP live-update stream or removes them; `packages/coordinator/{observations,router,scorer}.ts` keep their pure functions and tests with headers stating the production consumer is gone, `P71.4` owning the re-homing onto **agent reports** (`ARCH/ROUTING.md` §5). **And one contradiction recorded rather than hidden:** the A8 local OpenAI-compatible server (`src-tauri/src/openai_cmds.rs`) builds on the *vault* broker directly, so it survived this removal — but it is an EveryAIOS inference path, which `ARCH/ROUTING.md` §10 retires. It needs an explicit v1 decision (`P71.4`/`P71.7`); it is not silently kept as a feature.
- Next: `P71.4` (usage/cost as observation, incl. the A8 decision), `P71.5b` (Chief vocabulary — the delegation policy in `coordinator/src/chief.ts` and `cua.rs`'s `ChiefSpend`/`RUNTIME_PLANES`/`AgentRole`/`WorkerOutcome`), `P71.6a/b` (first-run agent discovery + the engine-optional proof), `P71.9a–h` (the UI surfaces), then `P70` release engineering and the verification pass. Note for `P71.5b`: `primary_chief` is still the legacy name the readiness work deliberately did **not** absorb.
...
- Landed in code (highlights):
  1. `scripts/check-arch-invariants.mjs` — 14 implied-invariant checks wired into the CI docs job + `pnpm check:arch`: CRED-1/2/3 (no TS credential custody), AUTH-1/2/3 (one auth vocabulary), SCHEMA-1 (canonical schema objects), DECIDE-1/2 (one decider; no hardcoded approval outside `#[cfg(test)]`), LAYER-1/2/3/4 (core-engine purity; eval outside the runtime; no TS search cascade in the turn loop; coordinator has no privileged IO), PURITY-1/2/3/4 (crate-boundary manifests), TS-DUP + RUST-DUP (canonical records/ids declared once).
  2. Defect repairs: ACP permission path → host `PermissionGate` + fail-closed default (C1); `ClientMediation` seam for `fs/*`/`terminal/*` (C2); mediated default `GovernancePreference::Mediated` (C3); TS provider vault → handle-only façade with custody Rust-side + `GovernedSearchTransport` for the two key-holding search providers (C4); registry auth from the handshake, not the license (C7); registry `env` wired (C8); missing platform target skipped, never launched bare (C9); `license_url` + exact SPDX (C10); one auth wire contract (C11); registry `Ask` consent end-to-end (C12).
  3. Ownership consolidation: `AgentDirectory` (D1); advisory policy in `core-engine`, Guard the only decider (D3/D5/D6); `ContextManager` (D7); core-engine purity (D8); kernel-owned retrieval (D9/D10); one event store / Work model (D11); subagents as child Work/Runs (D14); `Execution` alias deleted (D21); coordinator boundary enforced (D22); namespace unified to `@everyaios/*` (D23); UI projections classified (D24); canonical schema layer (D25); purity rows (D26–D28, D31); verify contract moved upstream of the harness (D32).
  4. `AgentBinding` (B2): record aligned field-for-field to `ARCH/AGENT.md` §3 / `ARCH/CORE.md` §7.2; four lifecycle events + usage deltas durable over the journal; creation ≠ activation; `dead` terminal; “exactly one active per Session” enforced; plugin manifest’s colliding `AgentBinding` renamed `PluginAgentBinding`.
  5. `gen-codebase-map.mjs --check` fixed (the generator embedded its own byte/line counts, so the check could never pass).
- Docs: `TODO.md` live count corrected to the file’s real numbers (1305 done + 302 open); `ARCH/CORE.md` §7.5/§11, `ARCH/EXTERNAL-AGENTS.md` §6/§10, `ARCH/SECURITY.md` and `docs/codebase/` carry dated “repaired in code (unverified)” annotations; `SPEC-CHANGELOG.md` has the matching 2026-09-21 entry; `CODEBASE-MAP.md` regenerated.
- Next: 1) a verification pass over the wave before any row flips from `IMPLEMENTED — unverified` to `DONE`; 2) `P69.B3` `AgentAdapter` contract + negotiated capability matrix (contracts already read; upstream protocol shapes to be fetched from the ACP registry/schema, not assumed); 3) the remaining B-objects (B1, B4–B17), the E-gates (E2–E6, E9–E11), then P70 release engineering.

**(Previous, 2026-09-19 — V1 Acceptance Evidence Suites (Tracks 2/3/5) + Windows-Incomplete Status)**:
...
**(Previous, 2026-09-18 — Secret and ambient credential protection + MCP sandbox flag hardening)**:
- Scope:
