...
## 1. Active Goal
**(Current, 2026-09-18 — P64.6 candidate-aware shadow preflight landed end-to-end; committed)**:
- Scope: edits are typechecked in an **isolated shadow tree (staged candidate)** before landing — Rust staging + wire parsing + coordinator gate at the exact-edit call site. Docs synced. Committed.
- Files (6 modified): `crates/everyaios-core/src/execution.rs` (+~347: `ShadowCandidateFile`, `parse_shadow_candidate` fail-closed (≤32 files / workspace-relative / no `..` / ≤512 KiB), `stage_shadow_tree` + `ShadowCleanup`, `run_preflight_with_candidate` (empty candidate → legacy in-place path), dispatch parses `candidateFiles` from `execution/preflight` params, refusal → `verified:false` **no receipt**), `crates/everyaios-core/src/lib.rs` (re-exports), `packages/coordinator/src/tools.ts` (`runShadowPreflight({candidateFiles})`, omitted from the wire payload when absent; `applyExactEdit` computes post-state → preflights with the candidate **before** the mutating `tool/commit`, refuses on `preflightBlocks`), `packages/coordinator/src/p64-lane.test.ts` (+3 gate cases + candidate forwarding cases), `TODO.md` (P64.6 → `[PARTIAL 2026-09-18]`, checkbox stays open), `SPEC-CHANGELOG.md` (2026-09-18 entry).
- Verified: `cargo test -p everyaios-core --lib` **707 passed / 0 failed** (25 in `execution`, incl. new `p64_shadow_preflight_checks_the_candidate_not_the_tree`: a `broken` candidate fails, a `fixed` one passes, live root never written) · `cargo fmt -p everyaios-core -- --check` clean · `cargo clippy -p everyaios-core --all-targets -- -D warnings` exit 0 · coordinator `bun test` **397 passed / 0 fail / 1299 expects** + `npx tsc --noEmit` 0 · `check-doc-sync.mjs` exit 0 (166 caps, 1429 = 1221 + 208) · `ipc-parity.mjs` exit 0.
- Gotchas: `verified:false` is **no evidence, never a refusal** — only a ran-and-failed verdict (`preflightBlocks`) blocks; don't widen that. Worktree cleanup falls back to plain `remove_dir_all`; a cleanup failure only annotates `reason`, never upgrades a verdict. `structural`/`destructive` are **caller inputs to `decide_shadow_preflight`**, deliberately NOT added to any model-visible tool schema (a model-supplied risk flag would let the model lower its own gate). In the applyExactEdit harness the **read** commits before the preflight — assert on `file_ops.write` absence, not `tool/commit` absence. Candidate staging exercised on Linux only.
- Next: 1) expose a model-facing apply path that declares multi-file/structural/destructive risk (first-class, not a model arg) so a production edit earns a preflight → close P64.6; 2) surface `record_preflight` receipts in the P64.7 checkpoint/rollback timeline; 3) Windows run of the `git worktree add --detach` staging path (P68.7 host gap).
