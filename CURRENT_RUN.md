...
- Next: `P71.2c` (the sidecar reasoning path — `core-engine` loop + stages, `ConversationEngine`, `ChatRelay`'s `provider/stream` arm) and `P71.2f` (catalogue narrowing), then `P71.4`, `P71.5`, `P71.6`, `P71.9`. `P71.2c` is the wave's one large surgery: it must move the composer/picker to ACP-only in the same change (the built-in turn function and its Rust broker go together). Note for `P71.5b`: `primary_chief` is still the legacy name the readiness work deliberately did **not** absorb.
