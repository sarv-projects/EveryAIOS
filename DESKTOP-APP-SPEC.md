...
> **Version:** v3.23 · **Date:** 2026-08-18 (hostile-review + execution-kernel pass 2026-08-19) · **Supersedes:** v3.22 (2026-08-17)
...
**Key technical locks (from ARCH/02 + research spec §3):** SQLite + **sqlite-vec** (`vec0`, pre-v1 — pin version + migration tests + fallback if unavailable) + **FTS5** + **Rust-native graph store** (**LadybugDB** optional/deferred — Kuzu community fork, Kuzu abandoned Oct 2025) + **SQLCipher** (tokens) · **rquickjs/QuickJS-NG** script eval (~300µs instantiation upstream benchmark; ES2020 per rquickjs docs — QuickJS-NG aims at latest ECMAScript) · **`modelcontextprotocol/rust-sdk`** MCP server (Streamable HTTP, 2026-07-28 stateless spec — no sessions/initialize, every request self-contained via `_meta`, one endpoint; **ACP keeps `initialize`/session lifecycle — do not conflate the two protocols**; ⚠️ treat 2026-07-28 stateless behavior as the *new-transport* default, not universal — keep stdio + legacy stateful-HTTP support behind a per-transport/per-version adapter (2026-08-19 hostile-review note); **⚠️ chosen not wired — `everyaios-mcp` today is a serde tool-catalog only (serde + serde_json, no rust-sdk dep); the real rust-sdk server lands P6.7 — annotated 2026-08-18**) · **CDP over system Chrome/Edge** (only WebView2 exposes CDP; macOS/Linux webviews don't — hence child-process browser) · 64MB/512KB/30s run limits · watchdog re-armed per byte.
...
> **Current build state (full landing log in `SPEC-CHANGELOG.md`):** the 16-crate Rust core, Tauri shell, coordinator sidecar, and UI v2 are landed and tested — P0–P4 (browser/office/storage/PDF), P5 memory (7 algorithms + write-side consolidation), P6 orchestration (blueprints, sub-agents, plan executor, scheduler + monitoring), P7 guard/audit/codeintel, P8 eval (EV1), E15–E17, F8/F12/J17 ACP bridge, and the security-posture + harness-pattern passes. The **one open gate is Stage 0 — the live guard-gated tool executor** (`tool/list`/`tool/exec`/`tool/commit`): guard tickets, plan stepping, and the call-sites are built + tested + wired over `guard/*`, but the coordinator's real file/browser/shell tool-invocation runtime is the remaining harness seam. Everything else is sequenced behind it (see §7 + TODO).
...
- ❌ **Marketing the benchmark's "broadest control plane" claim before the ticketed executor is live** — doc-80 §8 conditions 1–5 = doc-82 Gate A = roadmap Stage 0; no campaign or fact-sheet may state the full trust claim until the tool-executor seam (spec §6 Current-build-state note, TODO Stage 0) is wired and receipt/recovery evidence exists.
...
**R1 · Approval fatigue (Guard-2 prompts on everything).** ⚠️ Partially true — the design already prevents it: Trust Ladder tiers (ARCH/06 §6.2) = reads never prompt, local-write-in-workspace auto ≥25, external writes ≥75 + diff-card, destructive **always** diff-card; Guard-1 blocks deterministic-bad patterns without a card; `permissions.toml` `min_confidence_for_auto`; ticket `approval_source` = `auto_ladder | guard1_pass | guard2_human | policy` (only `guard2_human` shows a card). *Fix:* land **Stage 0** (the live executor — §6 Current-build-state note) so the policy actually runs; ship tuned defaults; add **card-frequency telemetry** (reject-rate → auto-downgrade autonomy); "Do-It-With-Me"/quiet-mode (P28 ADD-3) is the same answer. Never auto-approve destructive (invariant).
...
**Current-state honesty (all external reviews consolidated — full detail in `SPEC-CHANGELOG.md`):** three independent reviews (the "benchmark gaps", the 28-finding runtime audit, and the hostile spec review + two "Agent OS" manifestos) reach the same single conclusion this section states: **Stage 0 — the live guard-gated tool executor — is the one real implementation gap**; cloud-continuity, audio digests, and pixel computer-use are deliberate, sequenced trade-offs, not misses. The invariant is already enforced at every call-site that exists today (ticket `Approved` state, `Allow`-carries-ticket, guard surface split, mandatory Excel/ACP tickets, CSP, WebMCP auth, fail-closed topo sort — TODO HARDENING H1/H2); Stage 0 makes it *live*. The hostile review's one factual defect (matrix 🔵 58 vs 59) is fixed, and the genuinely-missing execution-kernel seams (unified `Execution` record · lease fencing · per-run context snapshot · data-egress engine · idempotency keys · generated status manifest · R0–R4 tiers · Stage-0 adapter contract) are now TODO HARDENING H3.

### 9.2 Edge cases & failure modes (the hard cases the design must survive)

The invariants in §9 only count if the system behaves correctly at the edges. These are the explicit edge cases every executor and adapter is built against (each is tracked in TODO HARDENING H3 or the section noted):
...
> Adopted from the external benchmark review (doc 80), the non-model moat roadmap (doc 81), and the innovation-priority decisions (doc 82) — all web-verified 2026-08-17. **Everything in this section is gated on Stage 0 — the live ticketed executor** (the open tool-executor seam, §6 = the same item as TODO Stage 0). No new capability-matrix rows until implemented (K rows compose existing rows: J5/EV1/C6/C10/F8/I6/B7/E5/P7.7/D-series).
