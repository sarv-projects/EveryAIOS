...
**Explicitly already-covered (never re-queue):** native-Rust sidecar = TODO P29 (sift-core <20MB precedent validates the ~15MB target); binary serialization (MessagePack-class) = R4 "measure first" — this spec and the perf review conclude identically; SSE for long-running tasks = MRTR (server.rs); ACP-default for agent communication = Dynamic Chief (P38, §4.2.5a). **Perf-review re-check (2026-08-26):** P29 11/11 + P39 5/5 are landed — the earlier perf-research re-statement's Tier-1/2 flow is already in Rust; the only unqueued deltas it raised are **not architectural** and are queued measurement-gated as **TODO P45** (R4 applies to each): SQLite pragma tuning (non-vault), `mmap_size`, WAL checkpoint throttling, connection pool, honesty-preserving audit-NDJSON write batching, `content-visibility:auto`, provider DNS cache, 60s sidecar idle-kill, UI memo/worker/speculative-prefetch. (IronCalc dirty-cell recalc + CDP command pipelining were also named — not yet queued; lower value, pieced as P45/P-final call only if profiling justifies them.)
