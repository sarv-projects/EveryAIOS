...
1. **“Clean up my Downloads folder.”** It groups, finds duplicates, proposes folders and names, asks before moves, then shows what changed.
2. **“I have a job interview tomorrow. Get me ready.”** Calendar + notes + company research + a brief — one plan, then Start.
3. **“Research this company and make me a presentation.”** Browse, gather sources, write a deck, ask before overwriting a file.
4. **“I don’t know what to do with these 400 files. Organize them.”** Understand, group, rename, move — you see the plan first.

The person never has to know about D9 storage intelligence, Guard-1, tickets, or FTS5.

### Power users (Pro / Control Center)

Same coworker. The center can expand: plan → agents → resources → tickets → evidence → result.

1. **“Fix this repo.”** Inspect, smallest change, tests, diff card, commit — you can watch Terminal / Files / Browser in the right rail.
2. **“Refresh Q3 numbers and patch the exec summary.”** Spreadsheet recalc + paragraph rewrite, each write a Guard-2 card.
3. **Drive Claude Code / Codex next to the inbuilt engine** on the same folder, same approvals, same timeline — Settings → Intelligence, not the Home screen.
...
> **Version:** v3.44 · **Date:** 2026-08-22 (P8.8 packaging/updater/CI + P8.9 E2E-sync live TCP transport + v3.39 ExecutionKernel fields landed and versioned; **Office ⏸ ON HOLD** per user directive) · **Supersedes:** v3.42 (2026-08-21)
...
| A5 | Local models — Ollama managed + llamafile single-binary + **MLX (Mac, Rapid-MLX — doc 61)**; **agent-native class (doc 61): Muse Glimmer (30B dense, 120K ctx) + Nemotron 3.5 Lightning (30B MoE/3B active)** → retire the 15–20K ctx warning for this class; **doc 58:** llmfit hardware-fit picker before spawn — `recommend --json`; **UI (LM Studio 0.4.21 blocks, EveryAIOS chrome, `ui/` only):** Settings → Local models = Discover (**live Hugging Face Hub GGUF search — no hardcoded model names**) + My models (installed ollama/llamafile + hwfit fits/too-big) + Hardware (existing `local_hardware` RAM/cores/gpu + UI prefs). Download button **not wired**. **doc 79 / TODO P27 backend still open:** HF Range downloader, `index.json` store, `local://` broker resolve, GGUF runtime bind, richer hardware probes, LM Studio/llama.cpp process probe. **Never bake Hub repo ids into source.** | 🟢+🔵 |
...
| A7 | Asymmetric tiering — planner_model / subagent_models / depth=2 / concurrency=6 / writers=3 (**doc 59:** 13-factor weighted scorer + 4 mode packs + `auto/category:tier` DSL as the dynamic selection layer; **doc 62:** LangChain×Switchyard proof — 74% cheaper / 7% frontier calls across 145 tasks (Nemotron Lightning executor + Opus planner, escalate-by-floor not default); ACRouter C-A-F = post-v1 dynamic-learning tail; **doc 68 §4:** agent-scoped routing — hosted agents route on their own model surface, the native engine routes on intent-first tiers Fast/Quality/Private/Cheap). **Honesty (v3.39):** spec'd — 13-factor scorer & `RouteDecision`/`ProviderObservation` **not yet implemented**. Coordinator `router.ts` is a capability-filter + cost-sort (`selectModelForTask`); vault `tier.rs` has the strategy *vocabulary* (`lkgp` etc.), not a live scorer loop. No extra public OmniRoute strategy modes. | 🟡 |
...
| B3 | Sub-agents — role isolation, own context+workspace, DELEGATE_BLOCKED_TOOLS; **v3.39:** derived child permissions (parent ∩ deny ∩ explicit grants; task/todo default-deny — Kilo pattern) + Gemini-style termination/abort events (goal / timeout / max-turns / aborted / error) on the *inbuilt* executor. ACP already covers external agents (J17). **v3.40 Scout child (OpenCode, live-verified):** a built-in **read-only research** subagent — clones a dependency repo into a managed cache (path-floored, not the user workspace), inspects upstream source, returns a summary; never writes the project tree. Cheap model by default. | 🟢+🟡 |
...
| C6 | Knowledge graph store — Rust-native embedded graph (LadybugDB = optional/deferred Kuzu-fork backend, C++, ACID, Cypher, vector+FTS), temporal edges; **v3.39:** every edge carries EXTRACTED vs INFERRED + source span (file/line) — temporal windows already exist | 🟡 (P5.2 ✅ — Rust-native graph store + schema + spreading activation + depth-cap; LadybugDB C++ FFI deferred) |
| C7 | Memory injection — warm set ~0ms TTFT (target), scope-leakage floors, budgets | 🟢+🟡 |
| C8 | Sync/export/wipe — E2E-encrypted sync (opt-in — **landed 2026-08-22: protocol + live TCP transport, LAN/Tailscale**), export, per-scope wipe; **doc 61:** Obsidian-compatible `.md` memory mirror (`[[wiki-link]]`s) + 20-min auto-fetch cadence (OpenHuman pattern — a view/export surface, not a second store) | 🟢 |
...
### D. Office & files (user-critical) — ⏸ **ON HOLD (2026-08-22 user directive)**
> **Office is frozen.** The D1–D8 engine + D9–D12 storage intelligence are **landed and stay landed** (P4.1–P4.8, tested). **No further Office work** — no new Office UI, no LOKit/Google-Docs tier, no perfectness-gap follow-ups — until the hold is lifted. Rows below record the *landed* status; the engine stays as-built.
| ID | Capability | Status |
|---|---|---|
...
| D4 | **PDF open+edit** — render (pdf.js), form-fill/annotate (pdf-lib), text-swap (lopdf), redact, re-author; **v3.40 persist contract:** form-fill round-trip uses pdf.js `annotationStorage` (fill → save → reopen keeps values); pdf-lib page-granular copy keeps untouched pages intact | 🟡 (P4.4 ✅; pdf.js renderer → P4.7) |
...
| D10 | **Duplicate detection by hash** — 7-stage pipeline (size → xxHash3 prefix/suffix → BLAKE3), hardlink-aware, optional reflink (btrfs/xfs/apfs), group reports (fclones + eDirStat ordering, doc 49); **v3.40:** persistent hash cache + delta re-scan (fclones `--cache` pattern — first scan hours, re-scans minutes) | 🟡 (P4.8 ✅) |
...
| E5 | Session replay — injected recorder → NDJSON → SQLite; scrubber UI; has_gap; **v3.40:** optional Playwright-class HAR capture beside the event log (network ground truth for `has_gap`) | 🔵 (P2.10 ✅) |
...
| E14 | Behavioral realism — humanized input events (Bézier mouse curves, typing cadence), optional per-site; **v3.39:** learned browser helpers persist as skills under `agent-workspace/` (browser-harness pattern — P24), not one-shot scripts | 🔵 (P2.9 ✅) |
...
| G7 | **Instant filename/content search** — SQLite FTS5 filename index + notify-watcher incremental updates + optional OS-native hooks (Everything/MFT, mdfind, Baloo); Everything/UltraSearch UX, cross-platform (doc 49); **v3.40:** FTS5 trigram tokenizer for partial names; Windows incremental = **USN journal** (not per-folder `ReadDirectoryChangeW`); external-content FTS so the index does not duplicate path blobs | 🟡 (P4.8 ✅) |
...
| H5 | Office editors — docx/xlsx/pptx/pdf views + chat overlay (**doc 58:** evaluate Univer SDK as the office surface — Sheets first, Docs next, Slides last; OSS/Pro split) — ⏸ ON HOLD (2026-08-22: viewer surfaces landed P4.7; no further Office UI work until lifted) | 🟡 (P4.7 ✅; pdf.js canvas + notes panel → follow-up — **frozen by hold**) |
...
- **Workspace tabs beside chat** (Open WebUI Computer validation): **Editor** (**CodeMirror 6** over real disk — lock 2026-08-21: cockpit interop, not an IDE replacement; Monaco is heavier and copies VS Code) · **Files** (browse/upload/preview) · **Terminal** (run/stream/send-input/return-later) · **Git** (review diffs, stage, commit; **v3.40:** one verified surgical edit = one atomic commit — Aider unit, the concrete K2 reversible change) — the whole machine, real files, real shell, real processes, no sandbox fakes.
...
- Computer-use of native desktop apps is **required (E9)** — ChatGPT + Claude parity, Guard-2 gated; **not built yet**. Browser CDP is already the web path.
...
> **Current build state (full landing log in `SPEC-CHANGELOG.md`):** the 16-crate Rust core, Tauri shell, coordinator sidecar, and UI v2 are landed and tested — P0–P4 (browser/office/storage/PDF), P5 memory (7 algorithms + write-side consolidation), P6 orchestration (blueprints, sub-agents, plan executor, scheduler + monitoring), P7 guard/audit/codeintel, P8 eval (EV1), E15–E17, F8/F12/J17 ACP bridge, the security-posture + harness-pattern passes, **and — landed 2026-08-20 — Stage 0, the live guard-gated tool executor**: `tool/list` (the `ToolRegistry`: 42 MCP catalog + `script.run`/`file_ops.*`/`search.query`), `tool/exec` (Guard-1 pre-scan → `GuardService::evaluate` → ticket; read-only auto-Allow; canonical args-hash drift blocked), `tool/commit` (`use_ticket` + TOCTOU re-verify + dispatch + Merkle audit row + idempotency key), the coordinator `tools.ts` `ToolExecutor` (ask→wait→commit, never auto-consumes, sanitization, loop cap), native model function-calling (S0.3), plan-tool wiring (S0.4), UI tool chips + retry (S0.5), and the security invariants (S0.6 — TOCTOU binds, Chrome 136+ attach pairing, egress engine, R0–R4 tiers) + red-team/path/URL fuzz + ask→approve E2E + EV1 `eval/verify` (S0.7). **Remaining after Stage 0:** browser/office/search engines *attach* at commit (dispatch exists, honest "session not attached"/"G8 not built"); ExecutionKernel disk persistence + K1 receipts remain §10. **v3.39 named-type contracts (no new matrix IDs, not a rebuild):** (1) `ManagedResource` trait over existing process managers; (2) P27 as already specified + LM Studio/llama.cpp process probe; (3) `RouteDecision` + `ProviderObservation` on A7/H9; (4) `MCPServerRecord` on F6/F8; (5) ExecutionKernel fields — immutable `config_hash`/runtime manifest per run (incl. chat turns), pending Guard-2 inside the checkpoint, never-started vs started-unknown repair, event-sourced session (append-only log is source of truth; model history is a projection; fork = lineage at a completed turn); (6) `DocumentAsset` provenance + C6 edge confidence/source-span + B3 derived child permissions + inbuilt subagent abort/termination events + C2/C7 branch memory + `maintain()` tools + C3 abortable retrieval + E14 learned helpers + E10 acquisition adapter. **Priority:** engine-attach → ExecutionKernel disk → K1 → P27, then these contracts smallest-first. E9 stays ⚪. **v3.40** (existing rows only): MCP resources/elicitation/sampling; I6 executor hooks; B3 Scout; CodeMirror 6; F12 Cursor/Gemini honesty; D4 persist; I8 atomic commit; E2 diagnostics; E5 HAR; D10/G7 incremental. **Landed 2026-08-21 (P8.1–P8.8 + P3/P6 connectors):** the personality system (`everyaios-blueprint::persona`), opt-in telemetry (`everyaios-core::telemetry`), export/wipe (`everyaios-core::export`), the WSL/POSIX bridge (`everyaios-core::wsl`), the universal reader (`everyaios-core::reader`), the widget cards (`everyaios-core::widgets`), the search & research suite (new `everyaios-search` crate — G8 cascade, deep research, multi-channel adapters, cited reports, parallel fetch cascade, site index, repo-wide scan, all over the injected `SearchTransport` seam), the Blueprint editor + local-server UI views (`ui/src/components/views/`), the RSS measurement harness (`everyaios-core::rss_measure`), **P3 cockpit/audit UI** (scrubber + screenshot strip + search + watch mode + Stop in `audit-view.tsx`, cockpit slide-over with agent cards/interrupts/tokens in `cockpit-slideover.tsx`, STOP/UNDO wired to coordinator), **P3.3 traceparent propagation** (`stream_provider` injects `traceparent` into every broker HTTP request via `Broker::with_extra_headers`), and **P6 connector transports** (Gmail API, Google Calendar, IMAP/SMTP, browser-session Gmail/Notion/Linear/Outlook — all behind injectable `HttpTransport`/`CdpSession`/`MailTransport` seams, 24 tests green). **Landed 2026-08-22 (v3.44):** the **v3.39 ExecutionKernel fields** — immutable `config_hash`/`RuntimeManifest` per run (`execution.rs`), `PendingApproval` inside the checkpoint, `started_unknown_repair()` (never-started vs started-unknown), event-sourced session (append-only log is source of truth, model history a projection, `fork_at_turn` lineage) — all landed; **P8.8 packaging/updater/CI** — `tauri.conf.json` NSIS/WiX/deb/rpm/AppImage/dmg targets + updater config (minisign pubkey, endpoints, passive install), `tauri-plugin-updater` + `updater_cmds.rs` (check/install/relaunch), `.github/workflows/release.yml` 3-platform build matrix (deb/rpm/AppImage · NSIS/MSI · dmg) publishing updater-signed artifacts with the coordinator sidecar injected as a bundle resource, signing keypair gitignored in `.tauri/`; **P8.9 E2E-encrypted sync — protocol + live TCP transport** — `everyaios-core::sync` (ChaCha20-Poly1305 AEAD envelope with header-as-AAD, X25519 key exchange + confirm handshake, version vectors + tombstones, 3-way reconcile → `SyncDiff`, `SyncBundle` export/import, `SyncTransport` seam) + `sync_transport.rs` (length-prefixed TCP framing, ECDH handshake, `SyncServer` on 47615, `sync_with_peer`, LAN/Tailscale), wired as 8 Tauri commands + Settings → Connections UI (`SyncSection`). Everything else is sequenced behind those (see §7 + TODO).
...
6. Office engine (D1–D8): docx block-patch, IronCalc xlsx sidecar + deterministic planner, pptx part-editor, pdf suite, LibreOffice conformance oracle — **⏸ ON HOLD (2026-08-22): engine landed (P4.1–P4.8); no further Office work until lifted**
...
14. Widget cards (H17) · **desktop computer-use E9 (required, not built)** · remote handoff (H18) · WASM sandbox (I3) · HTML→video
...
| **P9** | **E9 desktop computer-use (required ChatGPT+Claude parity, not built)** + WASM sandbox (I3), connector sync→RAG, HTML→video, remote handoff (H18), local OpenAI server (A8), AutomationBench, CLI, **image generation (A10)**, **clipboard (H26)**, **voice TTS (H28)** | — |
...
2. **Trusted action loop** — "Nothing meaningful happens without scope, proof, and an approve/undo path." (K1 receipts + K2 change sets, after Stage 0; **v3.40:** the reversible unit for code is one verified I8 edit = one git commit)
