...
- **EV · Evaluation & honesty** — the EV1 verification suite: golden tests, conformance evidence, anti-hallucination checks — the product refuses to claim what it cannot show. **Disposition (honest, 2026-08-18):** `everyaios-eval` (55 tests) is currently a **dev/test-time gate** — it runs under `cargo test`, and no coordinator/Tauri runtime path calls it yet; wiring EV1 into a product surface (e.g. verified-completion receipts in the K1 work receipt) is pending the Stage-0 executor, not claimed.
...
> **Totals (counted from this file's rows):** 149 rows · 🟢 26 · 🟡 68 · 🔵 58 · ⚪ 12 (multi-status rows counted in every bucket; ARCH/09 is counted separately from its own rows and may differ where a row's status differs between files — reconciled 2026-08-18: A8 🟡→⚪, F5 🟢→⚪ per the Connector-platform decision). 33 algorithms: 17 built (🔁 retest) + 16 new.
...
| J3 | Guard-2 diff cards — native click-to-approve, human-gated (web sensitive-action confirm dialogs included; the approve command is webview-callable, so nonce hardening is a P7.5 item) | 🔵 |
...
> **🔁 TO BE RETESTED** = shipped as tested TypeScript in `APP/packages/` (mobile-verified) — **must re-run its test suite + re-benchmark on the desktop runtime before it counts as done here.** The desktop target changes I/O (webview IPC, sidecar process model, Rust browser layer), so "shipped" ≠ "verified here". **Disposition (honest, 2026-08-18):** the C-series memory algorithms (1–4, 6, 8–11) were **re-implemented natively in Rust** as `everyaios-memory` (168 tests, wired into the coordinator via `memory/*` JSON-RPC) rather than retested in the TS sidecar — the Rust-native reimplementation **supersedes** the 🔁 sidecar retest for those. The remaining TS-side algorithms (5, 7, 12–17) still need the **coordinator-runtime retest** (part of the Stage-0 tool-runtime seam): today the coordinator imports core-* only in `chat.ts`/`plan.ts`/`prompt.ts`/`catalog.ts` (core-engine/core-ai/core-files/core-providers) — `core-memory`, `core-search`, `core-automations`, `core-tools` are **not** consumed at runtime yet, so their 🔁 flags stand.
...
**R1 · Approval fatigue (Guard-2 prompts on everything).** ⚠️ Partially true — the design already prevents it: Trust Ladder tiers (ARCH/06 §6.2) = reads never prompt, local-write-in-workspace auto ≥25, external writes ≥75 + diff-card, destructive **always** diff-card; Guard-1 blocks deterministic-bad patterns without a card; `permissions.toml` `min_confidence_for_auto`; ticket `approval_source` = `auto_ladder | guard1_pass | guard2_human | policy` (only `guard2_human` shows a card). *Fix:* land **Stage 0** (the live executor — §6 "Remaining") so the policy actually runs; ship tuned defaults; add **card-frequency telemetry** (reject-rate → auto-downgrade autonomy); "Do-It-With-Me"/quiet-mode (P28 ADD-3) is the same answer. Never auto-approve destructive (invariant).
...
**R7 · Vault key derivation is still the dev placeholder (review C2).** ✅ True and already labeled in code (`default_vault_key()` → `"everyaios-core-dev-key-do-not-use"` when `EVERYAIOS_VAULT_KEY` is unset, marked "Not for production — replaced by the P1.1 key-management design"). The key-ring/OAuth/Session-Vault *storage* landed (P1.1–P1.7), but the **key derivation** (user passphrase + keyfile + KDF → SQLCipher) is **not yet shipped** — until it is, a user who skips `EVERYAIOS_VAULT_KEY` has a vault encrypted with a known string. *Fix:* ship P1.1 key derivation (passphrase/keyfile → KDF) + a first-boot key-setup gate **before** any release; the broker/vault isolation is unaffected (this is a key-derivation gap, not a keys-leak-to-TS gap).
...
**Benchmark-gap status (the four items another review called "critical gaps" — all already tracked):** ① **Stage 0 live executor** = the #1 tracked item (spec §6 "Remaining", TODO P6/P7, P28 Stage 0, doc-80 condition 1, doc-82 Gate A) — the only one that is a real *implementation* gap. ② **Cloud continuity (lid-closed)** = a **deliberate local-first trade** (spec §1: "loses on … cloud-continue-when-lid-closed (H18 ⚪)"), with the optional **user-operated continuity node** in P28 Stage 5 (never a hidden founder service). ③ **Audio digests (H31/H28)** = real roadmap items, deferred post-v1 (H31 🟡 corpus-research + audio digest rides H28 TTS ⚪). ④ **Pixel computer use (E9)** = deliberate §8 non-goal for v1 (DOM/CDP/LSP is cheaper + more reliable), sequenced post-v1 dual-guard-gated via the P21 OpenAdapt demo-compiler queue. **None is an unplanned miss; ②③④ are sequenced/deliberate, ① is the gate everything else waits on.**

**Security-posture resolution (2026-08-19):** a separate adversarial code review of the *live runtime* (not the design) confirmed several findings that were actual code bugs — not the already-tracked risks above. The 🔴 real ones are now fixed and land in TODO HARDENING H1: ticket state machine gains `Approved` (`use_ticket` refuses `Pending`), `Allow` carries a ticket (ticket-every-effect), guard surface split (`handle_sidecar` vs `handle`), `min_confidence_for_auto` wired, Excel/ACP commits require a mandatory ticket, CSP restored, WebMCP auth + limits, and fail-closed topo sort. These are the *code-level* enforcement of the same invariant this section describes — the "one ticket → one executor → one event log" axiom is no longer bypassable at the call-sites that exist today (the Stage 0 executor is still the remaining gap to make it *live*).
