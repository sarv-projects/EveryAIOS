...
| chatboxai/chatbox | 41,600 | ⬛ | **Electron+React BYOK desktop client (GPL-3.0 — patterns only)** — multi-provider (OpenAI/Gemini/Anthropic/Ollama), provider Spotlight, models.dev registry, collapsible branches, minimap, Work Mode, cross-platform (Win/Mac/Linux) (docs 18/26/86; canonical move from `Bin-Huang/chatbox`, 2026-09-04) |
...
| andrewyng/openworker | 17,300 | ⬛ | **Andrew Ng's AI coworker (Python+Node+Rust)** — Tauri desktop + local agent server (aisuite), 25+ connectors (GitHub/Slack/Jira/Notion/Gmail/Calendar+MCP), approval-gated actions, scheduled automations, BYO model (11 providers+Ollama), local-first privacy, Rust toolchain (docs 40/83/86; v0.2.1: Skills, memory boards, reviewer auto-approve) |
| open-webui/desktop | 2,700 | 🟪 | **Open WebUI Desktop — official Electron wrapper (AGPL-3.0, early-alpha v0.0.20)** + unofficial MIT Tauri fork `reecelikesramen/open-webui-desktop` — multi-connection switcher, Spotlight screenshot, push-to-talk (doc 86; was Svelte-only note, doc 40) |
| OpenCoworkAI/open-cowork | 2,104 | 🟪 | **Desktop AI agent (TS/Electron, MIT)** — BYOK provider table + Ollama discovery + diagnostics, Skills hot-reload, MCP (npx-resolution + OAuth), in-process scheduler, artifacts file-steps; **engine = pi-agent loop since v3.0.0 (was Claude-Code-based — corrected doc 86)**; sandbox default-off host-exec + opt-in WSL2/Lima; store key hardcoded (do NOT copy — OS-keychain-bind instead); Win+Mac (docs 40/75/86) |
...
| mozilla-ai/llamafile | 25,900 | ⬛ | **Single-file local LLM runtime** (Mozilla — moved from `Mozilla-Ocho`, doc 87) — one binary = weights + server; lightest possible local-model path for BYO-local |
...
## Section 34 — doc 83 (competitor batch, 2026-08-17)
| skalesapp/skales | 1.6K | ⚪ | **Personal AI desktop agent (BSL 1.1 CLOSED — observe only; repo = distribution + historical v7 snapshot).** Electron+Next.js, ~300MB, ReAct loop + 180+ tools, multi-agent delegation, per-turn tool budgeting. Casual-user surface: `/goal` background-with-resume, `/code` folder-bound + inline diffs + one-click undo, QR phone pairing, AIPointer quick-ask overlay, Desktop Buddy / Iris Orbit (voice + 55 languages + pixel pets), bi-temporal memory + "Dreaming" overnight consolidation + Dream Diary, migration importer (OpenClaw/Hermes/ChatGPT), 15+ providers + Skales IQ trial + Ollama offline | **product-surface targets only** (P30.12–16: AIPointer quick-ask, /goal background, migration importer, Dreaming visibility; companion layer defer) — no code steal (BSL) |

## Section 35 — doc 86 (desktop deep-dive, 2026-09-04)
| openchamber/openchamber | ~9.5K | 🟪 | **OpenCode-powered supervisor (MIT)** — Session Goals (finish-line + per-turn check + survive-close), Multi-run (1 task × ≤5 models × worktrees) + Fusion, Changes Walkthrough, ops rail w/ per-session tokens/cost, Private Relay QR pairing, skills catalog (git sources, collision triage) | **STEAL** (TODO P51.9/P51.10/P51.26: goals, multi-run+fusion, catalog) |
| fathah/hermes-desktop | 14.1K | 🟪 | **Unaffiliated community Hermes companion (MIT) — UX reference ONLY, never a dependency.** Guided onboard, broadest provider presets, token/cost footer + `/usage`, 22 slash commands. Parallel 8642/SSE protocol diverges from canonical 9119/WS; unsigned installers | **REF-UX only** (TODO P51: onboarding + `/usage` footer pattern) |
| openclaw/acpx | 3.2K | 🟪 | **Headless ACP CLI (MIT)** — named parallel sessions + queue + `--cwd` scope + json/quiet/text + `flow run` TS workflows, 17-harness alias normalization, Codex CODEX_HOME isolation, doctor gate | **STEAL** (TODO P51.19: acpx shape for F12) |
| NousResearch/Hermes-Bot-Mode | 659 | ⚪ | **ARCHIVED 2026-08-17 — now bundled in-tree** at `hermes-agent/apps/desktop/src/plugins/hermes-bots/` (Bot=profile forever-chat, routines as namespaced cron, bot-to-bot DMs). Note only, do not track separately | tracked under hermes-agent |

## Section 36 — doc 87 (local-model runtimes + servers, 2026-09-04)
| nomic-ai/gpt4all | 77.4K | 🟪 | **Qt local-chat desktop (MIT)** — Explore HF search + Installed table (Filesize/RAM/Params/Quant/License/MD5) + Device selector + opt-in localhost `:4891` (4-endpoint OpenAI subset, no LAN) | **STEAL** (P52: Installed table, localhost-only default) |
| ml-explore/mlx | 28.3K | 🟪 | **Apple unified-memory array framework (MIT)** — NumPy-like Python + C++/Swift, lazy, CPU/GPU shared, no copies; macOS + Linux CUDA/CPU | **REF** (foundation under mlx-lm) |
| ml-explore/mlx-lm | 6.9K | 🟪 | **MLX LLM toolkit (MIT)** — `generate\|chat\|convert -q\|server\|cache_prompt\|benchmark`; `mlx-community/*-4bit` HF breadth; `--max-kv-size`, prefill-step, KV wiring limit | **STEAL** (P52.7: `mlx_lm.server` Mac sidecar + convert + prompt-cache) |
| lmstudio-ai/mlx-engine | 1.2K | 🟪 | **MLX backend for LM Studio (MIT)** — wraps `mlx-lm + Outlines` (structured) + `mlx-vlm` (vision); speculative draft-model demo | **WATCH** (structured output, speculative) |
| jundot/omlx | 21.4K | 🟪 | **Mac menubar OpenAI server (Apache-2.0)** — `:8000/v1` + `/admin` (monitor/mgmt/chat/benchmark); tiered KV (RAM+SSD, survives restart); LRU + pin + per-model TTL + memory guard; one-click HF MLX downloader | **STEAL** (P52.6: pin/TTL/LRU + benchmark; closest Mac-native reference) |
| microsoft/Foundry-Local | 2.5K | 🟪 | **~20MB ONNX runtime (MIT SDK)** — curated versioned catalog; `download(progress%)→load()`; auto HW (NPU/GPU/CPU) + best-variant select | **STEAL** (P52.5: auto best-variant one-click) |
| LostRuins/koboldcpp | 11.6K | 🟪 | **Single-file exe + GUI (AGPL — patterns only)** — presets, GPU layers; `:5001` multi-API shim (Kobold+OpenAI+Ollama+A1111) | **STEAL** (P52.3: multi-API-shim idea); IGNORE rest |
| abetlen/llama-cpp-python | 10.6K | 🟪 | **Python binding + server (MIT)** — `from_pretrained` HF glob + chat-format registry + multimodal handlers + OpenAPI multi-model | **STEAL** (P52: HF-glob fallback pattern; primary stays Rust bind) |
| ggml-org/llama.cpp | 127K | 🟪 | **Reference inference engine (MIT)** — `tools/server/` (OpenAI + Anthropic Messages + rerank + parallel decode + multimodal + grammar + `--tools/--mcp-servers`); `llama serve -hf`; `--fit/--fit-target/--fit-ctx`, `-ngl`, `-ctk/ctv` | **STEAL** (P52.3/P52.4: serve shape, flags, fit fallback) |
| vllm-project/vllm | 91K | 🟪 | **Server-class inference (Apache-2.0)** — PagedAttention, cont. batching, prefix cache, TP/PP, LoRA; needs server GPUs | **IGNORE** for desktop; WATCH prefix-cache/batching ideas |
...
## Summary
- **296 repos** tracked (227 through section 26 + **19 new in section 27** + **1 new in section 28** + **8 new in section 29** + **19 new in section 31** + **4 new in section 32** + **3 new in section 33** + **1 new in section 34** + **4 new in section 35** + **10 new in section 36**) · **295 live** · **2 transferred** (`LearningCircuit/ai-data-science-team` → `business-science/ai-data-science-team`, 5,369⭐; `Mozilla-Ocho/llamafile` → `mozilla-ai/llamafile` (org move, doc 87))
...
- **New repos (doc 83):** **1 new** (skalesapp/skales 1.6k★, BSL 1.1 closed-source — personal AI desktop agent, observe-only) → **added as section 34** (cloned, README+product-surface read, 2026-08-17) → **282 total**; **depth upgrades: openworker 🟪→⬛ (code-level), deepseek-harness 🟩→⬛ (code-level), cc-switch ⬛ re-read (config-writer steal); steal queue → TODO P30 (risk×mode matrix, shell-op hardening, HarnessConfigWriter, model-visible-means-logged, profile/bundle layering, capability seams, waterfalls + build = AIPointer//goal/migration/Dreaming)**
- **New repos (doc 86):** **4 new** (openchamber/openchamber ~9.5K★ MIT — OpenCode supervisor: goals/multi-run+fusion/walkthrough; fathah/hermes-desktop 14.1K★ MIT — unaffiliated community UX reference only, parallel protocol, never a dependency; openclaw/acpx 3.2K★ MIT — headless ACP CLI; NousResearch/Hermes-Bot-Mode 659★ MIT — ARCHIVED into the hermes-agent tree, note only) + **1 canonical move** (`Bin-Huang/chatbox` → `chatboxai/chatbox` 41.6K★ GPL-3.0 Electron, count-neutral) → **added as section 35** (web-verified 2026-09-04, no installs) → **286 total**; **star refreshes:** hermes-agent 226K→241K, anomalyco/opencode 194K→203.8K, andrewyng/openworker 13.3K→17.3K, holaboss-ai/holaOS 6.1K→11.1K; **steal queue → TODO P51**; ⚠️ license discipline (Cherry AGPL / Chatbox GPL = patterns-only; Crush FSL / holaOS Modified-Apache = observe-only)
- **Biggest live repos:** openclaw 385K, superpowers 268K, ECC 238K, hermes 241K, anomalyco/opencode 203.8K, n8n 199.5K, AutoGPT 186K, markitdown 172K, anthropics/skills 167K, firecrawl 162K
