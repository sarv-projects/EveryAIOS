...
| **lightpanda-io/lightpanda** (Zig headless, AGPL) | `lightpanda serve` Chrome-compatible CDP + containment flags (`--block-private-networks`, `--disable-workers`, `--cdp-max-connections`) | **Tier-1 light engine (opt-in)** — spawn-only (AGPL, never linked), ~16× less memory than Chrome (🟡 ADAPT, doc 55) | `tiers.rs` Lightpanda branch (P2.4 ✅ — LIVE-verified against installed binary: JS-render fetch of example.com) |
| **obscurabrowser/obscura** (Rust, 21K★, MIT) | `obscura serve` + own CDP server (14 domains) + `LP.getMarkdown` + embedded MCP 32 tools + SSRF/file:// defaults + ~30MB RSS (doc 55, source-verified) | **Tier-1 light engine (default)** — SSRF/file:///connection defaults to copy; same Chrome-compatible CDP surface as Lightpanda (🟡 ADAPT, doc 55) | `tiers.rs` Obscura branch (P2.4 ✅ — spawn + flags; binary not installed here → clean BinaryNotFound → E8-escalates, LIVE-verified vs real Chrome) |
