...
- [x] `[DONE]` Test: PoW challenge auto-solved locally — **23 challenge tests: detection (case-insensitive, PoW-preferred), routing (PoW→local even with BYO; managed→human/BYO/visual), solve/verify round-trip + difficulty-sensitivity + out-of-range, surface/pending/single-use registry, grounding-request + option/point/unsolvable parse, createTask/poll/solve (ready/processing/error/timeout) + provider parse — 73/73 core tests, workspace 326, clippy 0**
