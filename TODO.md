...
- [ ] `[NOT DONE]` Build Windows installer (.msi via WiX or .exe via NSIS) — **build/release infra (needs Windows CI runner)**
- [ ] `[NOT DONE]` Build macOS .dmg + .app (code sign + notarize) — **build/release infra (needs macOS + Apple Developer cert)**
- [ ] `[NOT DONE]` Build Linux .deb + .rpm + .AppImage — **build/release infra (needs Linux cross-compile toolchain)**
- [ ] `[NOT DONE]` Implement auto-updater (Tauri built-in) — **Tauri updater config + signing key (release seam)**
...
- [ ] `[NOT DONE]` CI: build matrix for all 3 platforms — **build/release infra (needs per-OS runners)**

### P8.9 Sync / Export / Wipe (C8 — v2.0 §P8)
- [ ] `[NOT DONE]` E2E-encrypted memory/message sync (opt-in, LAN/Tailscale/own server) — **runtime seam: needs network transport + key exchange**
...
- [x] `[DONE — picker chrome]` Agent picker merges `acp_agents` + install/connect. **Send is always `chatStream` (H4). Default store agent is `'claude-code'`, not inbuilt. Model column is not hidden for ACP. Remaining: `available_commands` surface.**
