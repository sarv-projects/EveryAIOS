...
- [ ] `[NOT DONE]` **G9 read-cleaner (doc 64 §4 — brave adblock crate):** pre-`read`/`snapshot`/markdown-export transform — strip ads/trackers/consent-walls via `adblock` crate (brave/adblock-rust v0.13.0 MIT): `FilterSet` (bundled + user lists via `add_filter_list_with_permissions`), `Engine::matches` (url/hostname/initiator/request_type/third_party → `BlockerResult`), `hidden_class_id_selectors` + `url_cosmetic_resources` (DOM cleanup), `get_csp_directives` (response hardening), `serialize`/`deserialize` (compiled-engine cache); plus domain/network blocklist feed for the F11 containment layer — **new `everyaios-content` module in `everyaios-browser` (or standalone crate), row G9**
- [ ] `[NOT DONE]` **E16 WebMCP tool-cancellation (doc 64 §6.2 — lightpanda `cdp/domains/webmcp.zig`):** `invokeTool`/`cancelInvocation` — a first-class cancel for long-running page tools; error taxonomy distinguishes **`Cancelled` (-32001, caller aborted — clients must not retry) vs `Timeout` (-32002, tool exceeded deadline — tool-state outcome)** (lightpanda `mcp/protocol.zig`) — `everyaios-mcp` + `webmcp` follow-on
- [ ] `[NOT DONE]` **E16 slim-mode interactivity upgrade (doc 64 §6.3 — lightpanda `interactive.zig`):** add the **event-listener detection step** (elements clickable only via JS — SPA divs with onClick) to the slim-mode actionability classifier, alongside native-tag / ARIA-role / contenteditable / tabindex — `everyaios-browser::tree` slim mode follow-on
