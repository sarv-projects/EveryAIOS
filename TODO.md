...
- [ ] `[NOT DONE]` **Scheduler monitoring wiring + broader scheduling** (P6.4). The remaining pieces beyond the Rust core: (a) coordinator `scheduler.ts` should call `scheduler/monitor` after a monitoring run — extract an observation from the chat result + pass `conditionMet` (the "scheduler ≠ agent" split: Rust owns WHEN + state, the coordinator's LLM pass owns the semantic "meaningful change" judgment); (b) broader time windows (morning/afternoon/evening) as a schedule primitive above raw cron; (c) task↔chat association cascade — deleting a chat pauses its scheduled jobs (`Job.session_id` is the link; the cascade isn't wired yet).
