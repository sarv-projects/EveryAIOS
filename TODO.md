...
>
> **UI v2 migration note (2026-08-16):** the v2 cockpit (`ui/`, UI-DESIGN-PROMPT.md = canonical spec) **replaced** the v1 router pages. Historical `[DONE]` lines below that cite `ui/src/pages/*.tsx` (Chat, Cockpit, Spend, Trajectory, Audit, Spreadsheet, DocxViewer, PptxViewer, PdfViewer, Settings) describe the v1 implementation — the capability map is: Chat → `components/chat/*` (panel/composer/picker) + `lib/bridge.ts` streaming; Cockpit → GuardPanel + bridge ticket cards + status bar; Spend → composer budget strip + status-bar cache + AnalyticsPanel; Audit → `views/audit-view`; Spreadsheet → `views/office-xlsx-view`; Docx/Pptx/Pdf viewers → `views/office-*` (demo grid/paper/stage until the `office.ts` bridge is wired); Settings → `panels/settings-panel`; Trajectory (J5) → **not yet ported** (v1-only; the audit-view scrubber is the closest v2 surface — TODO P5.9). Remaining UI seams: office viewers + connectors panel live wiring (`office.ts`/`cockpit.ts` bridges preserved in `lib/`), ACP `authMethods` sign-in surface in the picker, Trajectory J5 port.
