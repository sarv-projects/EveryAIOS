...
- [ ] `[NOT DONE]` **Broader scheduling + task↔chat cascade** (P6.4). The remaining pieces after the monitoring core + coordinator wiring: (a) broader time windows (morning/afternoon/evening) as a schedule primitive above raw cron; (b) task↔chat association cascade — deleting a chat pauses its scheduled jobs (`Job.session_id` is the link; the cascade isn't wired yet); (c) UI push/badge for the `monitor` event (the coordinator emits the verdict; the UI notification surface is the follow-up).
...
### H3 Execution-kernel unification + hostile-review gates (external architecture reviews 2026-08-19 — open)
...
- [ ] `[NOT DONE]` **Unified `Execution` record — "everything is an Execution"** (H3/Stage 0). One durable unit (id · parent_id · workspace/session · trigger · objective · state · plan · policy_snapshot · capability_scope · context_snapshot · checkpoint · budget · event_stream · artifact_refs · approval_refs · verification · receipt) with a state machine (CREATED → PLANNING → READY → RUNNING {WAITING_TOOL · WAITING_APPROVAL · WAITING_USER · CHECKPOINTED} → VERIFYING → COMPLETED | FAILED/CANCELLED/PAUSED/RECOVERABLE) that chat turns, plans (P6.3), scheduler runs (P6.4), ACP agent runs (J17) and subagents (SubAgentRuntime) all enter — different triggers, same kernel. Currently plan + scheduler + chat keep separate run records; the unified record is what makes resume/fork/replay/handoff/audit/receipt work across all of them. Lands with Stage 0.
- [ ] `[NOT DONE]` **Fencing tokens on scheduler leases** (H3/P6.4). `RunState::Running` gains a fencing token so a stale worker cannot commit after lease expiry/reassignment — `lease_finish`/`lease_checkpoint` must reject a token that is no longer current (the hostile-review point: a lease proves liveness, not that the holder is still the owner).
- [ ] `[NOT DONE]` **Per-run context snapshot — Task/Occurrence/Run separation** (H3/P6.4). A scheduled/automated run snapshots its context + permission policy at run start (TaskDefinition → Occurrence(scheduled_at/timezone/missed_policy) → Run(run_id, task_version, permission_snapshot, context_snapshot) → Lease(worker_id, expiry, fencing_token) → Checkpoint(step, inputs_hash, outputs, evidence_refs)); "same conversation" becomes a UI projection / parent link, not the live session — so a user's live edits, a simultaneous run, or a changed policy cannot silently alter an in-flight automation.
- [ ] `[NOT DONE]` **Data Egress Engine — first-class module** (H3). Before any external call, a data-release plan (what data / where / why / which model / which user authorization → ALLOW / REDACT / DENY) with a per-execution egress inventory and a user-visible "data leaving device" surface; unifies the landed URL floors + browser network containment (§6.15) + P17 egress firewall into one engine, and formalizes the four explicit modes: **Offline** (no network/registries) · **Local** (local models/tools only) · **BYOK-connected** (direct provider calls) · **Third-party connected** (MCP/ACP/browser/registry/search deps) — each with its data destinations, credentials, update sources and failure behavior.
- [ ] `[NOT DONE]` **Per-capability idempotency keys** (H3/Stage 0). Tickets/executions carry `execution_id + action_id + idempotency_key`; external effects (email/calendar/payment/HTTP/browser) use provider idempotency keys where available, otherwise an explicit `uncertain` state + reconciliation before retry (extends the doc-53 §6.11 idempotency classes into the executor contract — never blindly replay a non-idempotent effect).
- [ ] `[NOT DONE]` **Generated capability status manifest** (H3, process). Replace hand-maintained status totals with a generated build manifest (commit SHA · test command + count · runtime-wired capability list · benchmark run ID · timestamp). The hostile review caught a real drift (matrix 🔵 58 vs actual 59); statuses must come from runtime evidence, not manual counting.
- [ ] `[NOT DONE]` **Named risk tiers R0–R4** (H3/guard). Formalize the existing DecisionPackage risk levels as named tiers — R0 harmless read (auto) · R1 reversible (auto per policy) · R2 external effect (approval) · R3 destructive (approval) · R4 high-privilege credential/network/system (explicit approval, deny-by-default) — with the tier on every capability definition + approval card.
- [ ] `[NOT DONE]` **Stage-0 adapter contract + exact-command install consent** (H3/Stage 0 + F8/P22). (a) Every executor/adapter implements `describe() → prepare() → authorize() → execute() → observe() → receipt()` (capability+risk+scopes / normalized op+preconditions+diff / ticket bound to op+preconditions / effect with cancellation+idempotency / postcondition verification / signed evidence refs); bind tickets to canonical op name + canonical paths + file identities + workspace/session/run + tool version + capability scope + network origin/resolved-IP + executable digest + precondition hash + expiry + nonce + approval source + single-use transition. (b) SEP-1024 exact-command consent: before any npx/uvx/install command, show the exact command + args and require consent; prefer verified native artifacts / OCI/WASM; no install scripts by default; SBOM + dependency lock + quarantine + rollback + revocation (extends the landed F8 sha256 pinning + P22 quarantine).
...
| **HARDENING Security + Harness Patterns** | **17** | **12** | **5** | **landed + ~1** |
...
| P6 Orchestration + Connectors | 95 | 46 | 49 | ~5 |
