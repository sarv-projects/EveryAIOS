...
**First-run / no bound agent** (corrected 2026-09-25 against `ADR/0005` and the live send path): the blocker is **no Ready agent bound** — not a missing provider key. With no bound agent the empty chat shows the honest empty state and the send path opens the agent-discovery/`SetupGate` with a stated reason (the real refusal is `unbound` — "No runnable agent bound", `ui/src/lib/bridge.ts`). Install comes from the ACP registry; **sign-in or set-up appears only for the methods that agent advertised**, and an empty `authMethods` list shows neither. EveryAIOS does not ask for a model key, because EveryAIOS does not choose the model. The gate states privacy + network destinations explicitly per path: an EveryAIOS-held key is encrypted in the local SQLCipher vault and sent only to that provider's API (no EveryAIOS server) and is used for **EveryAIOS's own connectors and tools**, never for the agent's reasoning; a local runtime download comes from huggingface.co and inference runs on the runtime the agent is pointed at.
