...
**Casual:** one discreet pill — `● Preview · demo data` (amber, plain-browser) / `● Ready · Local` or `● Processing…` (emerald/orange live-dot) + `🛡 100% Private (On-Device)` + `EveryAIOS v3.23`.
...
- **Power row:** mode pills (`Normal · Plan · Research · Quick · Code`, orange on active) · **agent/model picker** (see §4.8) · right mono cluster: `$spent / $cap` · `tokensK tok` · **context gauge** `{pct}% ctx` (amber ≥75%, red ≥90% with tooltip).
