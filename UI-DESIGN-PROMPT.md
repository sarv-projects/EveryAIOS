...
> **v3.75 / 2026-09-15 reconciliation (two-plane model — `ARCH/17`; its "frozen" status is lifted by [`ARCH/ADR/0003`](ARCH/ADR/0003-architecture-thaw-core-authority.md)):** this UI renders the **ownership split between the selected agent's plane and EveryAIOS's shared plane**. When the **built-in engine binding** is selected the cockpit shows the EveryAIOS provider/model surface **and** the shared EveryAIOS cowork capabilities (Office · Browser · Computer use · connectors · workspace map · artifacts); when an **external ACP agent** is selected the cockpit must present the agent's **own native** capabilities as the agent's (`Model · <agent>`, its own session/auth) and the EveryAIOS additions as **shared augmentation** — a user must be able to tell which layer a capability comes from without reading docs (ARCH/17 §17.5, `UX-TESTING-PLAN.md`). This is a labeling/ownership rule over surfaces already described below, not new chrome; the picker's ownership split is unchanged. *(No binding is privileged — the built-in one is one binding among equals, [`ARCH/AGENT.md`](ARCH/AGENT.md) §2.)*
...
- **Live hint popovers:** typing `/` lists **the pinned agent's** slash commands (inbuilt = EveryAIOS catalog; ACP = live `available_commands_update` — `/name` submitted as `session/prompt` text, never intercepted as EveryAIOS control while an external agent is pinned), `!` lists macros (inbuilt), `@` lists workspace file refs — filtered as you type, orange mono commands.
...
- **Stale chrome to fix (P58):** Keyboard cheat sheet must match `keyboard-shortcuts.tsx`; About must not hardcode `v0.7.2 · 2026.01.15`; Office flyout must not stamp `live: false` on working engines; status bar / picker must not use the static `AGENTS`/`MODELS` catalogs once a provider is ticked; composer `/` is ACP `available_commands` for an ACP-bound agent. **Resolved (P58.4):** the inbuilt table has one owner (`ui/src/lib/slash-commands.ts`) that the composer intercepts and Settings → Commands switches — never a second hand-typed list.
