...
**Rail.** Folder · Shell · Browse · Computer use · Code. Computer use = see-pane, not a fake OS. Progress = DAG. **Occupancy badge = currently picked Chief**, never `DEFAULT_ROUTING` (no “Browse is Grok, Shell is Codex” while the picker says Claude). · **Subagents** (B3: **+** installed CLIs only, editable when-to-use description; not a catalog of uninstalled names) · Launch CLI copy-cards · Chat & Auto-run (**H34: 🛡 Sandbox · 👀 Ask · ⚡ Auto · 🚀 Maximum** + local ctx)
...
- **Stale chrome to fix (P58):** Keyboard cheat sheet must match `keyboard-shortcuts.tsx`; About must not hardcode `v0.7.2 · 2026.01.15`; Office flyout must not stamp `live: false` on working engines; status bar / picker must not use the static `AGENTS`/`MODELS` catalogs once a provider is ticked; composer `/` is ACP `available_commands` for an ACP Chief. **Resolved (P58.4):** the inbuilt table has one owner (`ui/src/lib/slash-commands.ts`) that the composer intercepts and Settings → Commands switches — never a second hand-typed list.
...
Composer: Agent ▾ (installed Chiefs only) / Work Mode / Autonomy (H34) + Chief-dependent `/` `@`. Empty chat: Work/Code/Design intent, Desktop/Documents/Open folder/WSL/No project chips. Spec Q&A uses lettered MCQ cards. Right **Summary** view = progress timeline + empty Artifacts/References. Memory has a **Repo wiki** tab. Automations templates include Daily Brief / Weekly Review / Project Monitor. MCP disconnect uses a destructive toast.
