    ...
    pub fn builtin() -> Self {
        ...
        Self {
            agents: vec![
                ...
                acp("cline", "Cline", "Cline CLI — autonomous coding agent (cline --acp).", AuthMode::ApiKey, npx("cline", &["--acp"])),
                acp("opencode", "OpenCode", "Anomaly's open-source coding agent (opencode acp).", AuthMode::ApiKey, bin("opencode", &["acp"])),
                acp("hermes", "Hermes Agent", "Nous Research's self-improving agent (hermes acp).", AuthMode::ApiKey, bin("hermes", &["acp"])),
                acp("openclaw", "OpenClaw", "Personal AI with 100+ skills (openclaw client acp).", AuthMode::ApiKey, bin("openclaw", &["client", "acp"])),
                acp("qwen-code", "Qwen Code", "Alibaba's Qwen coding assistant.", AuthMode::ApiKey, npx("@qwen-code/qwen-code", &["--acp", "--experimental-skills"])),
                acp("goose", "goose", "Block's local, extensible, open source agent.", AuthMode::ApiKey, bin("goose", &["acp"])),
                acp("aider", "Aider", "AI pair programming in the terminal.", AuthMode::ApiKey, uvx("aider-chat", &[])),
                acp("kimi", "Kimi CLI", "Moonshot AI's coding assistant.", AuthMode::ApiKey, bin("kimi", &["acp"])),
                acp("kilo", "Kilo", "The open source coding agent (Kilo Code).", AuthMode::ApiKey, npx("@kilocode/cli", &["acp"])),
                // Zhipu's agent authenticates through the GLM Coding Plan — a
                // subscription, not local inference and not BYOK.
                acp("glm-agent", "GLM Agent", "Zhipu AI's GLM Coding Plan agent.", AuthMode::Subscription, npx("glm-acp-agent", &[])),
                acp("deepagents", "DeepAgents", "LangChain's batteries-included agent.", AuthMode::ApiKey, npx("deepagents-acp", &[])),
                acp("fast-agent", "fast-agent", "Multi-provider agent builder.", AuthMode::ApiKey, uvx("fast-agent-acp", &["-x"])),
                acp("minion-code", "Minion Code", "AI code assistant on the Minion framework.", AuthMode::ApiKey, uvx("minion-code", &["acp"])),
                acp("mistral-vibe", "Mistral Vibe", "Mistral's open-source coding assistant.", AuthMode::ApiKey, bin("vibe-acp", &[])),
                acp("dirac", "Dirac", "Cost-optimizing, fully open-source coding agent.", AuthMode::ApiKey, npx("dirac-cli", &["--acp"])),
                acp("stakpak", "Stakpak", "Open-source DevOps agent in Rust.", AuthMode::ApiKey, bin("stakpak", &["acp"])),
                acp("autohand", "Autohand Code", "Autohand AI's coding agent.", AuthMode::ApiKey, npx("@autohandai/autohand-acp", &[])),
                ...
                acp("amp", "Amp", "ACP wrapper for Amp, the frontier coding agent.", AuthMode::Subscription, bin("amp-acp", &[])),
                ...
                acp("harn", "Harn", "Harn runs .harn agent pipelines as an ACP agent.", AuthMode::Unknown, bin("harn", &["serve", "acp"])),
                acp("crow-cli", "crow-cli", "Minimal ACP-native coding agent.", AuthMode::Unknown, bin("crow-cli", &["acp"])),
                acp("vtcode", "VT Code", "Open-source agent with LLM-native understanding.", AuthMode::Unknown, bin("vtcode", &["acp"])),
                acp("corust-agent", "Corust Agent", "Co-building with a seasoned Rust partner.", AuthMode::Unknown, bin("corust-agent-acp", &[])),
                acp("agoragentic", "Agoragentic", "Agent marketplace with 174+ AI capabilities.", AuthMode::Unknown, npx("agoragentic-mcp", &["--acp"])),
                acp("commandcode", "Command Code", "Frontier coding agent that learns your taste (candidate — verify ACP flag via registry).", AuthMode::Unknown, bin("commandcode", &["acp"])),
                ...
                acp("pi", "Pi", "Minimal AI agent toolkit (via the pi-acp adapter).", AuthMode::ApiKey, npx("pi-acp", &[])),
            ],
        }
    }
