...
mod tests {
    ...
    fn native_loop_dispatches_without_framing() {
        let loop_ = NativeLoop::spawn(|cmd| match cmd {
            NativeCommand::Read { tool, .. } => NativeResponse::Ok {
                value: format!("read:{tool}"),
            },
            NativeCommand::Execute { ticket_id, .. } => {
                if ticket_id.is_empty() {
                    NativeResponse::Refused {
            ...
            NativeCommand::Health => NativeResponse::Health {
                rss_mb: 42,
                children: 0,
            },
        });
        let read = loop_
            .dispatch(NativeCommand::Read {
                tool: "snapshot".into(),
                args: String::new(),
            })
            .unwrap();
        assert_eq!(
            read,
            NativeResponse::Ok {
                value: "read:snapshot".into()
            }
        );
        let refused = loop_
            .dispatch(NativeCommand::Execute {
                tool: "fs.write".into(),
                args: String::new(),
    ...
    fn execute_requires_guard_ticket_end_to_end() {
        ...
        let loop_ = NativeLoop::spawn(move |cmd| match cmd {
            NativeCommand::Execute {
                ...
                Err(_) => NativeResponse::Refused {
                    reason: "ticket not consumable".into(),
                },
            },
            _ => NativeResponse::Refused {
